...
class GameplayApplicationMixin:
    ...
    def save_gameplays(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        ...
        return {"ok": True, "space_id": space_id, "gameplays": tavern.gameplay_definitions}
    ...
    def list_gameplay_sessions(
        ...
        *,
        ...
        self._ensure_visible(tavern, user_id)
        owner = self._is_owner(tavern, user_id)
        sessions = self.store.list_gameplay_sessions(space_id)
        ...
        return {"space_id": space_id, "sessions": [self._session_payload(session, include_events=False) for session in sessions]}

    def start_gameplay_session(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        if not user_id:
    ...
    def abandon_gameplay_session(self, space_id: str, session_id: str, user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_visible(tavern, user_id)
        session = self.store.get_gameplay_session(space_id, session_id)
        if not session:
