...
from ...domain.space_policy import can_view_memory, can_view_space, clean_text
...
class WorldInfoApplicationMixin:
    ...
    def list_world_info(self, user_id: str = "", space_id: str = "") -> dict[str, Any]:
        ...
        if space_id:
            ...
            source_taverns = self.store.list_spaces(include_private=bool(user_id), owner_id=user_id)
        ...
        for tavern in source_taverns:
            ...
            for entry in tavern.world_info:
                item = entry.to_dict()
                item["space_id"] = tavern.id
                item["space_name"] = tavern.name
                entries.append(item)
    ...
    def create_world_info(self, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        ...
        entry = self._world_info_entry_from_payload(space_id, payload)
    ...
    def update_world_info(self, entry_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        ...
        for index, entry in enumerate(tavern.world_info):
            if entry.id != entry_id:
                continue
            merged = {**entry.to_dict(), **payload, "id": entry_id, "space_id": space_id}
            tavern.world_info[index] = self._world_info_entry_from_payload(space_id, merged, entry_id=entry_id)
            self.store.update_space(tavern)
    ...
    def delete_world_info(self, entry_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        ...
        if len(tavern.world_info) == original_count:
            raise HTTPException(status_code=404, detail="WorldInfo entry not found")
        self.store.update_space(tavern)
        return {"ok": True, "entry_id": entry_id, "space_id": space_id}
    ...
    def test_world_info_global(self, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        ...
        return self.test_world_info(space_id, payload, user_id)
