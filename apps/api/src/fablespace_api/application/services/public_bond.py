...
from fablespace_api.core.public_bond import (
...
class MySQLPublicBondStore(PublicBondStore):
    ...
    def get_bond(self, bond_id: str) -> NpcPublicBond | None:
        from fablespace_api.infrastructure.models import NpcPublicBondModel
    ...
    def get_exclusive_active_bond_count(
        ...
        with self._db.session_scope() as session:
            return (
                session.query(NpcPublicBondModel)
                .filter(
                    NpcPublicBondModel.space_id == space_id,
...
class NpcPublicBondApplicationMixin:
    ...
    def apply_public_bond(
        ...
        eligibility = self.check_bond_eligibility(
            space_id, character_id, visitor_id, visitor_strength
        )
        ...
        existing_pending = bond_store.get_pending_bond(space_id, character_id, visitor_id)
        if existing_pending:
            raise HTTPException(status_code=409, detail="已有待审批的申请，请等待审批结果")

        active_bonds = bond_store.get_bond_by_visitor(space_id, character_id, visitor_id)
        ...
        if is_exclusive:
            # 检查是否已有活跃排他关系
            exclusive_count = bond_store.get_exclusive_active_bond_count(space_id, character_id)
            if exclusive_count > 0:
                # 入队
                existing_queue = bond_store.get_queue_entry(space_id, character_id, visitor_id)
                if existing_queue:
                ...
                # 获取当前最大 position
                queue = bond_store.get_queue_for_character(space_id, character_id)
    ...
    def approve_public_bond(
        ...
        # 若是从队列晋升而来，更新队列状态
        queue_entry = bond_store.get_queue_entry(space_id, character_id, bond.visitor_id)
        if queue_entry:
        ...
        if bond.bond_type.is_exclusive():
            ...
            if queue:
                ...
                pending_bond = bond_store.get_bond_by_visitor(
                    space_id, character_id, first_in_queue.visitor_id
                )
    ...
    def get_visitor_bond(
        ...
        bonds = bond_store.get_bond_by_visitor(space_id, character_id, visitor_id)
        eligibility = self.check_bond_eligibility(
            space_id, character_id, visitor_id, visitor_strength
        )
        ...
        queue_entry = bond_store.get_queue_entry(space_id, character_id, visitor_id)
    ...
    def get_public_bonds_for_character(
        ...
        bonds = bond_store.get_active_bonds_for_character(space_id, character_id)
    ...
    def cancel_queue_entry(
        ...
        pending_bond = bond_store.get_pending_bond(space_id, character_id, visitor_id)
        if pending_bond:
