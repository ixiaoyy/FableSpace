...
from fablespace_api.core.clue_hunt import (
...
class ClueHuntApplicationService:
    def __init__(self, store: ClueHuntStore, space_store: TavernStore):
        self.store = store
    ...
    def _validate_route_governance(self, route: ClueHuntRoute) -> None:
        ...
        for index, node in enumerate(route.nodes):
            if not node.space_id:
                raise HTTPException(status_code=400, detail=f"第 {index + 1} 站缺少 space_id")
            if not node.clue:
    ...
    def claim_reward(self, route_id: str, session_id: str, user_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "reward": {
                ...
                "scope": "tavern-local",
                "space_id": final_node.space_id if final_node else "",
            },
            "session": self._session_payload(route, session),
        }
