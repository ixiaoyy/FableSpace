...
from .territories import TerritoryApplicationService
...
class SpaceApplicationService(
    ...
    """Application facade for native `/api/v1/spaces` use cases.

    This layer deliberately depends on the current space/gameplay/memory core
    ...
    def from_settings(cls, settings: ApiSettings) -> "SpaceApplicationService":
        ...
        store = create_space_store(settings)
    ...
    def _ensure_owner(self, tavern: Space, user_id: str) -> None:
        if tavern.owner_id and tavern.owner_id != user_id:
            raise HTTPException(status_code=403, detail="你不是此空间的主人")

    def _ensure_visible(self, tavern: Space, user_id: str) -> None:
        if not can_view_space(tavern, user_id):
    ...
    def create_visitor_note(self, space_id: str, data: dict[str, Any], user_id: str) -> dict[str, Any]:
    ...
    def list_visitor_notes(self, space_id: str, user_id: str, limit: int = 20, offset: int = 0) -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_owner(tavern, self._require_user_id(user_id))
        if not self.visitor_note_store:
            raise HTTPException(status_code=500, detail="访客反馈存储不可用")
        notes, total = self.visitor_note_store.list_notes(space_id, limit=limit, offset=offset)
        return {"notes": notes, "count": total}

    def delete_visitor_note(self, space_id: str, note_id: str, user_id: str) -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        safe_user_id = self._require_user_id(user_id)
        if not self.visitor_note_store:
            raise HTTPException(status_code=500, detail="访客反馈存储不可用")
        note = self.visitor_note_store.get_note(space_id, note_id)
        if not note:
        ...
        self.visitor_note_store.delete_note(space_id, note_id)
...
# Backward-compatible service name while route modules are being renamed.
TavernApplicationService = SpaceApplicationService
