...
class VisitorNoteStore:
    ...
    def create_note(self, space_id: str, visitor_id: str, data: dict[str, Any]) -> dict[str, Any]:
        note = {
            "id": f"note_{secrets.token_hex(8)}",
            "space_id": _text(space_id, limit=128),
        ...
        all_notes.setdefault(note["space_id"], []).append(note)
        self._write_all(all_notes)
        return dict(note)

    def list_notes(self, space_id: str, *, limit: int = 20, offset: int = 0) -> tuple[list[dict[str, Any]], int]:
        notes = [dict(item) for item in self._read_all().get(str(space_id), [])]
    ...
    def get_note(self, space_id: str, note_id: str) -> dict[str, Any] | None:
        for note in self._read_all().get(str(space_id), []):
            if note.get("id") == note_id:
                return dict(note)
        return None

    def delete_note(self, space_id: str, note_id: str) -> bool:
        all_notes = self._read_all()
        notes = all_notes.get(str(space_id), [])
        kept = [note for note in notes if note.get("id") != note_id]
        if len(kept) == len(notes):
            return False
        all_notes[str(space_id)] = kept
        self._write_all(all_notes)
        return True
...
class SQLAlchemyVisitorNoteStore:
    ...
    def create_note(self, space_id: str, visitor_id: str, data: dict[str, Any]) -> dict[str, Any]:
        from datetime import UTC, datetime
        from fablespace_api.infrastructure.models import VisitorNoteModel
    ...
    def list_notes(self, space_id: str, *, limit: int = 20, offset: int = 0) -> tuple[list[dict[str, Any]], int]:
        ...
        safe_offset = max(0, int(offset or 0))
        with self.database.session_scope() as session:
            query = session.query(VisitorNoteModel).filter_by(space_id=str(space_id))
            total = query.count()
    ...
    def get_note(self, space_id: str, note_id: str) -> dict[str, Any] | None:
        ...
        with self.database.session_scope() as session:
            model = session.query(VisitorNoteModel).filter_by(space_id=str(space_id), id=str(note_id)).first()
    ...
    def delete_note(self, space_id: str, note_id: str) -> bool:
        ...
        with self.database.session_scope() as session:
            count = session.query(VisitorNoteModel).filter_by(space_id=str(space_id), id=str(note_id)).delete()
            return count > 0
