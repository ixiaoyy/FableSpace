...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m fablespace_api.core.bundle",
        description="Export a fixed-structure static bundle from a FableSpace world JSON.",
    )
          ...
          <div class=\"world-map-viewport\" id=\"world-map-viewport\" data-zoom-tier=\"survey\">
            <div class=\"world-map-canvas\">
              ...
              <svg id=\"observer-map\" viewBox=\"0 0 {viewport['width']} {viewport['height']}\" data-zoom-tier=\"survey\" role=\"img\" aria-labelledby=\"observer-map-title observer-map-desc\">
                <title id=\"observer-map-title\">{escape(str(showcase.get('title') or 'FableSpace Observer'))}</title>
...
def _render_preview_html(world: dict[str, Any], showcase: dict[str, Any], manifest: dict[str, Any]) -> str:
    ...
    translations = {
        "zh-CN": {
            "pageTitleSuffix": "FableSpace 预览页",
        ...
        "en": {
            "pageTitleSuffix": "FableSpace Preview",
...
<html lang=\"en\">
  <head>
    ...
    <title>{escape(showcase.get('title') or 'FableSpace Preview')}</title>
    <style>
  ...
  <body>
    ...
    <script>
      ...
      function readStoredLanguage() {{
        try {{
          return window.localStorage.getItem("fablespace-language") || "";
        }} catch (error) {{
          return "";
        }}
      }}
      ...
      function writeStoredLanguage(language) {{
        try {{
          window.localStorage.setItem("fablespace-language", language);
        }} catch (error) {{
