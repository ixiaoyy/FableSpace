...
class MemoryStore(ABC):
    ...
    @abstractmethod
    def list_atoms(self, space_id: str, **filters: Any) -> list[MemoryAtom]:
        """List atoms for a tavern with optional metadata filters."""

    @abstractmethod
    def get_atom(self, space_id: str, memory_id: str) -> MemoryAtom | None:
        """Return one atom by id."""

    @abstractmethod
    def save_atom(self, space_id: str, atom: MemoryAtom) -> MemoryAtom:
        """Create or update one atom."""

    @abstractmethod
    def delete_atom(self, space_id: str, memory_id: str) -> bool:
        """Delete one atom."""
...
class KeywordMemoryStore(MemoryStore):
    ...
    def list_atoms(self, space_id: str, **filters: Any) -> list[MemoryAtom]:
        atoms = self._backend_list(space_id)
    ...
    def get_atom(self, space_id: str, memory_id: str) -> MemoryAtom | None:
        if self.backend and hasattr(self.backend, "get_memory_atom"):
            atom = self.backend.get_memory_atom(space_id, memory_id)
            return atom if isinstance(atom, MemoryAtom) else _coerce_memory_atom(atom)
        return self._atoms.get(space_id, {}).get(memory_id)
    ...
    def save_atom(self, space_id: str, atom: MemoryAtom) -> MemoryAtom:
        ...
        if self.backend and hasattr(self.backend, "save_memory_atom"):
            saved = self.backend.save_memory_atom(space_id, atom)
            return saved if isinstance(saved, MemoryAtom) else MemoryAtom.from_dict(saved)
        self._atoms.setdefault(space_id, {})[atom.id] = atom
        return atom

    def delete_atom(self, space_id: str, memory_id: str) -> bool:
        if self.backend and hasattr(self.backend, "delete_memory_atom"):
            return bool(self.backend.delete_memory_atom(space_id, memory_id))
        atoms = self._atoms.get(space_id, {})
    ...
    def search_atoms(
        ...
        **filters: Any,
        ...
        atoms = self.list_atoms(space_id, **filters)
    ...
    def _backend_list(self, space_id: str) -> list[MemoryAtom]:
        if self.backend and hasattr(self.backend, "list_memory_atoms"):
            values = self.backend.list_memory_atoms(space_id)
        else:
            values = list(self._atoms.get(space_id, {}).values())
...
def auto_create_memories_from_chat(
    ...
    *,
    ...
    if not space_store or not space_id or not visitor_id:
        return []

    existing_atoms = []
    try:
        existing_atoms = space_store.list_memory_atoms(space_id)
    except Exception:
        existing_atoms = []
    ...
    for role, message_text, message_id in messages:
        for candidate_text in _split_memory_candidates(message_text):
            ...
            try:
                saved = space_store.save_memory_atom(space_id, atom_to_save)
            except Exception:
                continue
