...
from fablespace_api.domain.space_package_policy import (
    SPACE_PACKAGE_TYPE,
...
def _space_llm_config_to_client(space_config) -> "LLMConfig":
    """Convert tavern.py LLMConfig to llm_clients.LLMConfig."""
    from fablespace_api.core.llm_clients import LLMConfig
    return LLMConfig(
        backend=space_config.backend,
        model=space_config.model,
        api_key=space_config.api_key,
        base_url=space_config.base_url,
        temperature=space_config.temperature,
        max_tokens=space_config.max_tokens,
        top_p=space_config.top_p,
        frequency_penalty=getattr(space_config, 'frequency_penalty', 0.0),
        presence_penalty=getattr(space_config, 'presence_penalty', 0.0),
    )
...
def _safe_space_package_space(value: dict[str, Any]) -> dict[str, Any]:
    allowed = (
        "id",
        "name",
        "description",
...
def _package_list(package: dict[str, Any], space_payload: dict[str, Any], key: str) -> list[Any]:
...
def _package_dict(package: dict[str, Any], space_payload: dict[str, Any], key: str) -> dict[str, Any]:
    value = package.get(key)
...
def _is_space_owner_obj(tavern: Any, user_id: str) -> bool:
...
def _memory_atom_is_visible(atom: MemoryAtom, tavern: Any, user_id: str) -> bool:
    ...
    if atom.visibility == "owner":
        return _is_space_owner_obj(tavern, user_id) or _memory_subject_matches(atom, user_id)
...
def _memory_atom_is_editable(atom: MemoryAtom, tavern: Any, user_id: str) -> bool:
    if atom.visibility == "private":
        return _memory_subject_matches(atom, user_id)
    return _is_space_owner_obj(tavern, user_id) or bool(user_id and atom.created_by == user_id)
...
from fablespace_api.core.api_service import build_health_payload, build_meta_payload, build_nearby_payload
from fablespace_api.core.llm_clients import create_client, LLMError
...
from fablespace_api.core.prompt_builder import ChatMessage as PromptChatMessage, PromptBuildConfig, PromptBuilder
from fablespace_api.core.application.web_payloads import build_behavior_insights, build_orchestrate_payload, record_memory_graph_event
from fablespace_api.core.nearby import generate_nearby_preview
from fablespace_api.core.overpass import OverpassError
from fablespace_api.core.writeback import WritebackEngine, WritebackStore
from fablespace_api.core.orchestrator.rule_engine import RuleBasedOrchestrator
from fablespace_api.core.memory_graph import WorldMemoryGraph
from fablespace_api.core.dynamic_signals import inject_disturbance, clear_disturbance, get_disturbance
...
class WebService:
    def __init__(
        ...
        *,
        ...
        # Tavern service (new)
        self.space_store = space_store or TavernStoreCore(self.settings.output_root / "taverns")
        self.space_service = TavernServiceCore(self.space_store)
    ...
    def _state_cards_for_prompt(self, space_id: str) -> list[dict[str, Any]]:
        ...
        try:
            return [
                ...
                for card in self.space_store.list_state_cards(space_id)
            ]
        except Exception:
            return []
    ...
    def list_spaces_payload(
        ...
        taverns = self.space_service.list_spaces(
    ...
    def list_spaces(self, user_id: str = "", **filters: Any) -> list[dict[str, Any]]:
        """Compatibility wrapper for compatibility router endpoints."""
        return self.space_service.list_spaces(owner_id=user_id, **filters)

    def get_space_payload(self, space_id: str, user_id: str = "") -> dict[str, Any]:
        """Get a specific tavern by ID"""
        return self.space_service.get_space(space_id, user_id)

    def get_space(self, space_id: str, user_id: str = "") -> dict[str, Any]:
        """Compatibility wrapper for compatibility router endpoints."""
        return self.get_space_payload(space_id, user_id)

    def create_space_payload(self, data: dict[str, Any], owner_id: str = "") -> dict[str, Any]:
        """Create a new tavern"""
        owner_id = str(owner_id or "").strip()
        if not owner_id:
            raise HTTPException(status_code=401, detail="创建空间需要明确店主身份")
        return self.space_service.create_space(data, owner_id)

    def update_space_payload(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        """Update a tavern"""
        return self.space_service.update_space(space_id, data, user_id)

    def update_space(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        """Compatibility wrapper for compatibility router endpoints."""
        return self.update_space_payload(space_id, data, user_id)

    def get_share_payload(self, space_id: str, base_url: str = "", user_id: str = "") -> dict[str, Any]:
        """Get shareable info for a tavern (public data only)."""
        tavern = self.space_store.get_space(space_id)
        if not tavern:
            raise HTTPException(status_code=404, detail="空间不存在")
        if tavern.access == "private" and not _is_space_owner_obj(tavern, user_id):
            raise HTTPException(status_code=403, detail="此空间是私人的")
        return build_space_share_payload(tavern, base_url=base_url)

    # ─── Gameplay System ────────────────────────────────────────────────

    def _ensure_gameplay_space_visible(self, tavern: Any, user_id: str) -> None:
    ...
    def get_gameplays_payload(self, space_id: str, user_id: str = "") -> dict[str, Any]:
        tavern = self.space_store.get_space(space_id)
        if not tavern:
            raise HTTPException(status_code=404, detail="空间不存在")
        self._ensure_gameplay_space_visible(tavern, user_id)
    ...
    def save_gameplays_payload(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        tavern = self.space_store.get_space(space_id)
        if not tavern:
            raise HTTPException(status_code=404, detail="空间不存在")
        ...
        self.space_store.update_space(tavern)
        return {
            "ok": True,
            "space_id": space_id,
            "gameplays": gameplays,
    ...
    def list_gameplay_sessions_payload(
        ...
        *,
        ...
        sessions = self.space_store.list_gameplay_sessions(space_id)
        if not owner:
    ...
    def start_gameplay_session_payload(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        if not user_id:
        ...
        for session in self.space_store.list_gameplay_sessions(space_id):
            if (
        ...
        self.space_store.save_gameplay_session(space_id, session)
        return {
            "ok": True,
            "resumed": False,
            "session": self._session_payload(session),
    ...
    def _ai_director_result(
        ...
        llm_config = self.space_store.get_llm_config(tavern.id)
        ...
        def _complete(payload: dict[str, Any]) -> str:
            client = create_client(_space_llm_config_to_client(llm_config))
            prompt = [
                {
                    "role": "system",
                    "content": (
                        "你是 FableSpace 空间玩法的 AI Director。只返回 JSON，字段包括 "
    ...
    def advance_gameplay_session_payload(
        ...
        if not tavern:
            raise HTTPException(status_code=404, detail="空间不存在")
        session = self.space_store.get_gameplay_session(space_id, session_id)
        if not session:
    ...
    def abandon_gameplay_session_payload(self, space_id: str, session_id: str, user_id: str = "") -> dict[str, Any]:
        tavern = self.space_store.get_space(space_id)
        if not tavern:
            raise HTTPException(status_code=404, detail="空间不存在")
        session = self.space_store.get_gameplay_session(space_id, session_id)
    ...
    def test_world_info_payload(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        ...
        """
        tavern = self.get_space_payload(space_id, user_id)
    ...
    def get_output_rules_payload(self, space_id: str, user_id: str = "") -> dict[str, Any]:
    ...
    def save_output_rules_payload(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        ...
        tavern = self.space_store.update_space(tavern)
        return {
            "ok": True,
            "space_id": space_id,
    ...
    def test_output_rules_payload(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
    ...
    def get_prompt_blocks_payload(self, space_id: str, user_id: str = "") -> dict[str, Any]:
    ...
    def save_prompt_blocks_payload(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
    ...
    def preview_prompt_blocks_payload(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
    ...
    def get_runtime_presets_payload(self, space_id: str, user_id: str = "") -> dict[str, Any]:
    ...
    def save_runtime_presets_payload(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
    ...
    def apply_runtime_preset_payload(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        ...
        space_payload = self.update_space_payload(space_id, update_payload, user_id)
        return {
            "ok": True,
            "space_id": space_id,
            "active_preset_id": preset.get("id") or "",
            "preset": preset,
            "tavern": space_payload,
        }

    def delete_space_payload(self, space_id: str, user_id: str = "") -> dict[str, str]:
        """Delete a tavern"""
        return self.space_service.delete_space(space_id, user_id)

    # ── Voice Config ───────────────────────────────────────────────────

    def save_voice_config_payload(
        self, space_id: str, data: dict[str, Any], user_id: str = ""
    ) -> dict[str, Any]:
        ...
        from fablespace_api.core.space import VoiceConfig
        voice_config = VoiceConfig.from_dict(data)
        self.space_store.save_voice_config(space_id, voice_config)
    ...
    def get_voice_config_payload(
        ...
        vc = self.space_store.get_voice_config(space_id)
        if vc:
    ...
    def synthesize_voice_payload(
        ...
        llm_config = self.space_store.get_llm_config(space_id)
    ...
    def enter_space_payload(
        self, space_id: str, password: str = "", user_id: str = "", visitor_gender: str = ""
    ) -> dict[str, Any]:
        """Enter a tavern (verify password)"""
        return self.space_service.enter_space(space_id, password, user_id, visitor_gender)

    def list_space_visitors_payload(self, space_id: str, user_id: str = "") -> dict[str, Any]:
        ...
        for session in self.space_store.list_chat_sessions(space_id, limit=None):
        ...
        visitors = []
        for state in self.space_store.list_visitor_states(space_id):
    ...
    def list_memory_atoms_payload(
        ...
        *,
        ...
        atoms = []
        for atom in self.space_store.list_memory_atoms(space_id):
    ...
    def get_memory_atom_payload(self, space_id: str, memory_id: str, user_id: str = "") -> dict[str, Any]:
        ...
        atom = self.space_store.get_memory_atom(space_id, memory_id)
        if not atom:
            raise HTTPException(status_code=404, detail="记忆不存在")
        if not _memory_atom_is_visible(atom, tavern, user_id):
            raise HTTPException(status_code=403, detail="不能访问这条记忆")
        return {"space_id": space_id, "memory_atom": atom.to_dict()}

    def create_memory_atom_payload(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        ...
        if not tavern:
            raise HTTPException(status_code=404, detail="空间不存在")
        owner = _is_space_owner_obj(tavern, user_id)
        if tavern.access == "private" and not owner:
            raise HTTPException(status_code=403, detail="此空间是私人的")

        atom = _memory_atom_from_payload(data or {}, space_id=space_id, user_id=user_id)
        ...
        created = self.space_store.save_memory_atom(space_id, atom)
        return {"ok": True, "space_id": space_id, "memory_atom": created.to_dict()}
    ...
    def update_memory_atom_payload(
        ...
        existing = self.space_store.get_memory_atom(space_id, memory_id)
        if not existing:
            raise HTTPException(status_code=404, detail="记忆不存在")
        if not _memory_atom_is_editable(existing, tavern, user_id):
            raise HTTPException(status_code=403, detail="不能修改这条记忆")

        updated = _memory_atom_from_payload(data or {}, space_id=space_id, user_id=user_id, existing=existing)
        ...
        if updated.visibility == "private" and updated.scope.startswith("visitor_"):
            ...
            if private_ids != {user_id}:
                raise HTTPException(status_code=403, detail="私密访客记忆只能属于当前访客")
        if updated.scope.startswith("visitor_") and updated.visitor_id and updated.visitor_id != user_id and not _is_space_owner_obj(tavern, user_id):
            raise HTTPException(status_code=403, detail="不能把记忆转给其他访客")

        saved = self.space_store.save_memory_atom(space_id, updated)
        return {"ok": True, "space_id": space_id, "memory_atom": saved.to_dict()}

    def delete_memory_atom_payload(self, space_id: str, memory_id: str, user_id: str = "") -> dict[str, Any]:
        ...
        deleted = self.space_store.delete_memory_atom(space_id, memory_id)
        if not deleted:
            raise HTTPException(status_code=404, detail="记忆不存在")
        return {"ok": True, "space_id": space_id, "memory_id": memory_id}

    def add_character_payload(
        self, space_id: str, data: dict[str, Any], user_id: str = ""
    ) -> dict[str, Any]:
        """Add a character to a tavern"""
        return self.space_service.add_character(space_id, data, user_id)

    def update_character_payload(
        self, space_id: str, char_id: str, data: dict[str, Any], user_id: str = ""
    ) -> dict[str, Any]:
        """Update a character"""
        return self.space_service.update_character(space_id, char_id, data, user_id)

    def delete_character_payload(
        self, space_id: str, char_id: str, user_id: str = ""
    ) -> dict[str, str]:
        """Delete a character"""
        return self.space_service.delete_character(space_id, char_id, user_id)

    def import_character_card_payload(
        self, space_id: str, card_data: dict[str, Any], user_id: str = ""
    ) -> dict[str, Any]:
        """Import a SillyTavern character card"""
        return self.space_service.import_character_card(space_id, card_data, user_id)

    def export_space_package_payload(self, space_id: str, user_id: str = "") -> dict[str, Any]:
        """Export a shareable Space package without credentials or visitor data."""
        space = self.space_store.get_space(space_id)
        if not space:
            raise HTTPException(status_code=404, detail="空间不存在")
        if space.owner_id and space.owner_id != user_id:
            raise HTTPException(status_code=403, detail="你不是此空间的主人")

        space_payload = space.to_dict()
        safe_space = _safe_space_package_space(space_payload)
        safe_space.pop("password_hash", None)
        llm_preset = _safe_llm_preset(space_payload.get("llm_config"))
        return {
    ...
    def import_space_package_payload(self, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        """Import a shareable Space package as a new Space at a chosen coordinate."""
        payload = data or {}
        ...
        if isinstance(package.get("space"), dict):
            space_payload = package.get("space") or {}
        else:
        ...
        llm_preset = _safe_llm_preset(
            _package_dict(package, space_payload, "prompt_preset").get("llm_config")
            or space_payload.get("llm_config")
        )
        if llm_preset:
            create_payload["llm_config"] = llm_preset

        created = self.create_space_payload(create_payload, owner_id=user_id)

        update_payload = {
            "characters": _package_list(package, space_payload, "characters"),
            "world_info": _package_list(package, space_payload, "world_info"),
            "groups": _package_list(package, space_payload, "groups"),
            "bookmarks": _package_list(package, space_payload, "bookmarks"),
            "chat_templates": _package_list(package, space_payload, "chat_templates"),
            "gameplay_definitions": _package_list(package, space_payload, "gameplay_definitions"),
            "output_rules": _package_list(package, space_payload, "output_rules"),
            "prompt_blocks": _package_list(package, space_payload, "prompt_blocks"),
            "runtime_presets": _package_list(package, space_payload, "runtime_presets"),
            "skill_packs": _package_list(package, space_payload, "skill_packs"),
            "active_preset_id": str(package.get("active_preset_id") or space_payload.get("active_preset_id") or ""),
            "memory_policy": _package_dict(package, space_payload, "memory_policy"),
        }
        if llm_preset:
            update_payload["llm_config"] = llm_preset

        imported = self.update_space_payload(created["id"], update_payload, user_id)
        voice_config = _package_dict(package, space_payload, "voice_config")
        if voice_config:
            try:
                self.save_voice_config_payload(imported["id"], voice_config, user_id)
                imported = self.get_space_payload(imported["id"], user_id)
            except HTTPException:
                raise
            except Exception:
    ...
    def list_tavern_backups(self, space_id: str = "") -> dict[str, Any]:
        ...
        for file_path in files:
            if space_id and space_id not in file_path.name:
                continue
            backups.append({
                "name": file_path.name,
                "path": str(file_path),
    ...
    def create_tavern_backup(self, space_id: str, user_id: str = "") -> dict[str, Any]:
        tavern = self.space_store.get_space(space_id)
        if not tavern:
            raise HTTPException(status_code=404, detail="Tavern not found")
        if tavern.owner_id and tavern.owner_id != user_id:
        ...
        chat_sessions = self.space_store.list_chat_sessions(space_id, limit=None)
        payload = {
            "version": 1,
        ...
        backup_dir = self.settings.output_root / "backups"
        backup_dir.mkdir(parents=True, exist_ok=True)
        backup_name = f"backup_{_sanitize_snapshot_id(space_id)}_{_now_ms()}.json"
    ...
    def restore_tavern_backup(
        ...
        space_payload = backup_data.get("tavern", backup_data)
        target_space_id = space_id or space_payload.get("id", "")
        if not target_space_id:
            raise HTTPException(status_code=400, detail="space_id is required")

        existing = self.space_store.get_space(target_space_id)
        if existing:
            if existing.owner_id and existing.owner_id != user_id:
                raise HTTPException(status_code=403, detail="你不是此空间的主人")
            self.space_service.update_space(target_space_id, space_payload, user_id)
        else:
            from fablespace_api.core.space import Tavern
            restored_tavern = Tavern.from_dict({**space_payload, "id": target_space_id})
            ...
            self.space_store.create_space(restored_tavern)

        if replace_chats:
            self.space_store.delete_chat_history(target_space_id)
        ...
        for session in backup_data.get("chat_sessions", []):
            ...
            restored_messages += self.space_store.replace_chat_history(
                target_space_id,
                visitor_id,
                character_id,
                messages,
    ...
    def _build_tavern_character_prompt(
        ...
        *,
        ...
        space_id = getattr(tavern, "id", "")
        character_id = getattr(character, "id", "")
        prompt_user_name = visitor_display_name or visitor_id[:16] or "旅人"
        messages = history_messages if history_messages is not None else self.space_store.get_chat_history(
            space_id, visitor_id, character_id, limit=20
        )
        prompt_visitor_state = self.space_store.get_visitor_state(space_id, visitor_id) if visitor_id else None
        visitor_message_count = len(messages)
        if visitor_id:
            try:
                visitor_message_count = sum(
                    int(session.get("message_count", 0) or 0)
                    for session in self.space_store.list_chat_sessions(
                        space_id,
        ...
        if memory_policy.get("mode") in {"structured", "balanced", "long_context"}:
            try:
                visible_atoms = [
                    atom for atom in self.space_store.list_memory_atoms(space_id)
        ...
        config = PromptBuildConfig(
            space_name=getattr(tavern, "name", ""),
    ...
    def space_chat_payload(
        ...
        from fablespace_api.core.space import ChatMessage as TavernChatMessage, VisitorState, _normalize_gender
        ...
        llm_config = self.space_store.get_llm_config(space_id)
        if not llm_config or not llm_config.is_configured():
            self._mark_space_closed(tavern)
            return self._degraded_chat_payload(
                character_id=character_id,
                character_name=character.name,
                response_text="",
        ...
        if str(llm_config.backend or "").lower() in {"rules", "rule_based", "public_welfare"}:
            ...
            try:
                llm_client_config = _space_llm_config_to_client(llm_config)
                ...
                logger.warning("LLM call failed for tavern=%s character=%s: %s", space_id, character_id, e)
                return self._degraded_chat_payload(
                    character_id=character_id,
                    character_name=character.name,
                    response_text="",
        ...
        self.space_store.add_chat_message(
            TavernChatMessage(
                id=user_message_id,
                space_id=space_id,
                character_id=character_id,
        ...
        self.space_store.add_token_usage(space_id, token_count)

        updated_visitor_state = None
        if visitor_id:
            visitor_state = self.space_store.get_visitor_state(space_id, visitor_id) or VisitorState(
                visitor_id=visitor_id,
                space_id=space_id,
                first_visit=now,
            )
    ...
    def _mark_space_closed(self, tavern) -> None:
    ...
    def _count_tokens(
        ...
        from fablespace_api.core.token_counter import get_counter
    ...
    def test_llm_payload(
        ...
        from fablespace_api.core.llm_clients import create_client, LLMConfig, LLMError
        from fablespace_api.core.space import LLMConfig as TavernLLMConfig
        from fablespace_api.core.space import _hydrate_system_public_welfare_llm_config
    ...
    def space_chat_history_payload(
        self,
        space_id: str,
        visitor_id: str,
        ...
        messages = self.space_store.get_chat_history(
            space_id, visitor_id, character_id, limit=history_limit
        )
    ...
    def _inject_managed_spaces(self, payload: dict[str, Any], center_lat: float, center_lon: float, radius: int):
    ...
    def get_quick_reply_manager(self):
        ...
        if not hasattr(self, "_quick_reply_manager"):
            from fablespace_api.core.quick_replies import QuickReplyManager
    ...
    def get_command_manager(self):
        ...
        if not hasattr(self, "_command_manager"):
            from fablespace_api.core.slash_commands import get_command_manager
    ...
    def get_extension_manager(self):
        ...
        if not hasattr(self, "_extension_manager"):
            from fablespace_api.core.extensions import get_extension_manager
    ...
    def send_group_chat_payload(
        ...
        from fablespace_api.core.space import ChatMessage as TavernChatMessage, VisitorState
        ...
        self.space_store.add_chat_message(TavernChatMessage(
        ...
        for speaker in speakers:
            ...
            self.space_store.add_chat_message(tavern_msg)
        ...
        if total_token_count > 0:
            self.space_store.add_token_usage(space_id, total_token_count)
    ...
    def _group_chat_history_messages(self, tavern: Any, visitor_id: str, *, limit: int = 50) -> list[Any]:
        ...
        sessions = self.space_store.list_chat_sessions(
            tavern.id,
            visitor_id=visitor_id,
            limit=None,
        )
    ...
    def get_preset_manager(self):
        ...
        if not hasattr(self, "_preset_manager"):
            from fablespace_api.core.presets import PresetManager
...
def _download_remote_file(url: str, target_path: Path) -> None:
    request = Request(
        url,
        headers={
            "User-Agent": "FableSpaceSnapshot/1.0",
