...
from fablespace_api.core.memory.core import KeywordMemoryStore, MemoryAtom, MemorySearchResult, MemoryStore
...
class VectorMemoryStore(MemoryStore):
    ...
    def list_atoms(self, space_id: str, **filters: Any) -> list[MemoryAtom]:
        return self.fallback.list_atoms(space_id, **filters)

    def get_atom(self, space_id: str, memory_id: str) -> MemoryAtom | None:
        return self.fallback.get_atom(space_id, memory_id)

    def save_atom(self, space_id: str, atom: MemoryAtom) -> MemoryAtom:
        saved = self.fallback.save_atom(space_id, atom)
    ...
    def delete_atom(self, space_id: str, memory_id: str) -> bool:
        deleted = self.fallback.delete_atom(space_id, memory_id)
        if deleted:
    ...
    def search_atoms(
        ...
        **filters: Any,
        ...
        if not self.semantic_enabled or not str(query or "").strip():
            return self.fallback.search_atoms(space_id, query, limit=limit, **filters)
        ...
        try:
            vector_hits = self.vector_store.search(
                ...
                filter_fn=lambda entry: entry.metadata.get("space_id") == space_id,
            )
        except Exception:
            return self.fallback.search_atoms(space_id, query, limit=limit, **filters)
        ...
        for entry, score in vector_hits:
            ...
            if not memory_id or memory_id in seen:
                continue
            atom = self.fallback.get_atom(space_id, memory_id)
            if not atom:
                continue
