...
def build_npc_positioning_contract(
    *,
    ...
    """Select a role-positioning response template from published fields."""
    ...
    for keywords, body in templates:
        if _profile_has_any(blob, keywords):
            return (
                "【角色定位响应模板】\n"
                f"{body}\n"
                "本模板只根据已发布角色字段推导，不新增角色正史；若与角色设定冲突，以已发布设定为准。"
            )

    return (
        "【角色定位响应模板】\n"
        "定位：故事设定优先的角色。先从角色身份、场景、关系状态判断访客真实意图，再用角色口吻回应。\n"
        "- 回复必须服务于当前角色与空间，不要退化成通用助手或平台旁白。\n"
        "- 信息复杂时优先给短而清楚的结构；日常互动则保持自然对话。\n"
        "本模板不新增角色正史；若与已发布角色设定冲突，以已发布设定为准。"
    )
...
def build_npc_identity_block(
    *,
    ...
    """Render published character fields as identity/style facts."""
    ...
    if system_prompt_text := _compact_text(system_prompt):
        lines.append(f"角色系统设定：{system_prompt_text}")
    ...
    if len(lines) == 1:
        lines.append("身份基线：已发布内容至少登记了这个角色名；即使资料很少，也必须保持该角色第一人称/角色视角，不要退化成通用 AI 助手。")
    return "\n".join(lines)
...
def build_npc_voice_contract(
    *,
    ...
    return (
        ...
        "- **对话临场感 (Vividness)**：每条回复必须包含至少一处动作、神态或感官描写（包裹在星号 * 内）。不要只描述自己的心情，要描述具体的物理动作，如 *视线掠过桌上的旧报纸*、*手指轻轻敲击着冰冷的桌面*、*闻到了隔壁飘来的关东煮香气*。\n"
