...
import './story-progress-panel.css'
...
function latestNarration(session, scene) {
  ...
  return lastEvent?.narration || '故事正在推进。选择一个行动，或补充一句话继续这一幕。'
}
...
function stageForNodeId(nodeId, gameplay = null) {
  ...
  if (isFlowerbedGameplay(gameplay)) {
    ...
    if (normalized === 'trade') return '角色交换'
...
export default function StoryProgressPanel({ session, scene = {}, gameplay = null, busy = false, onChoice, onSubmit, onAbandon, onReturnToDoorway }) {
  ...
  const goal = gameplay?.goal || gameplay?.summary || ''
  ...
  return (
    <section className={`gameplay-session-panel ${completed ? 'is-complete' : ''}`} aria-label="故事进度">
      ...
      {completion ? (
        <div className="gameplay-session-panel__completion">
          ...
          <p>{completion.summary || '这段故事已经完成。'}</p>
