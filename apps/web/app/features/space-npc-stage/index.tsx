...
import type { RoleplayClaim, Space, SpaceCharacter } from "../../lib/spaces"
...
const NPC_VISUAL_STYLES: NpcVisualStyle[] = [
  ...
  },
  {
    ...
    title: "手绘幻想小镇",
    label: "旅人披肩 · 叶灯挂饰",
    spaceCue: "水彩木梁、小镇路牌、叶影灯和童话式回访记忆。",
...
function avatarUrlFor(character: SpaceCharacter): string {
  return (
...
function resolveNpcStyle(character: SpaceCharacter, space: Space, index: number): NpcVisualStyle {
...
function NpcPortrait({
  ...
  const fallbackPortrait = resolveNpcPortraitMatch(character, space, style.preferredArchetypes, index)
        ...
        >
          ...
          <div className="relative z-10 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-violet-50/72">{activeStyle.spaceCue}</p>
            </div>
            <span
        ...
        {characters.length ? (
          <div className="grid min-w-0 gap-3 sm:grid-cols-2">
            {characters.map((character, index) => {
              const style = resolveNpcStyle(character, space, index)
              ...
              return (
                ...
                >
                  <NpcPortrait character={character} space={space} style={style} index={index} />
