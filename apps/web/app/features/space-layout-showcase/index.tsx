...
import { SpaceNpcStage } from "../space-npc-stage"

type SpaceLayoutAction = {
  title: string
  text: string
  icon: string
  targetLayout?: string
...
type SpaceLayout = {
  ...
  description: string
  accent: string
  actions: SpaceLayoutAction[]
}

type SpaceLayoutStats = {
  location: string
  accessStatus: string
...
type SpaceLayoutShowcaseProps = {
  spaceId: string
  space: Space | null
  error: string
  characters: SpaceCharacter[]
  selectedCharacter?: SpaceCharacter
  selectedCharacterId: string
  roleplay: RoleplayState | null
  onSelectCharacter: (character: SpaceCharacter) => void
  shareSlot?: ReactNode
  creatorSlot?: ReactNode
}

type LayoutProps = SpaceLayoutShowcaseProps & {
  space: Space
  stats: SpaceLayoutStats
...
const layoutBackgroundImages: Record<string, string> = {
  lobby: spaceNeonImage,
  "npc-chat": npcDialogueImage,
  "quest-play": spaceNightImage,
  "hybrid-room": spaceStreetImage,
}
...
function layoutById(layoutId: string): SpaceLayout {
  return (SPACE_LAYOUTS.find((layout: SpaceLayout) => layout.id === layoutId) || SPACE_LAYOUTS[0]) as SpaceLayout
}

function accentFor(layout: SpaceLayout) {
...
function worldInfoPreviews(space: Space): WorldInfoPreview[] {
...
function TimeStatusBadge({ timeStatus }: { timeStatus: NonNullable<SpaceLayoutStats["timeStatus"]> }) {
...
function LayoutHeader({
  ...
  characters: SpaceCharacter[]
  selectedCharacter?: SpaceCharacter
  activeLayout: SpaceLayout
  activeLayoutId: string
  stats: SpaceLayoutStats
  onLayoutChange: (layoutId: string) => void
}) {
  const accent = accentFor(activeLayout)
  const backgroundImage = layoutBackgroundImages[activeLayout.id] || spaceNeonImage
  ...
  const doorwayScene = space
    ? previewText(toText(space.scene_prompt, space.description || "店主还没有写下场景提示。"), 118)
    : "从发现页选择一个真实坐标上的空间入口。"
  const worldPreview = space ? worldInfoPreviews(space)[0] : null
  const doorwayLocation = space?.address || stats.location
  ...
  return (
    <section className="relative min-w-0 overflow-hidden rounded-[2.4rem] border border-white/12 bg-slate-950/78 shadow-2xl shadow-black/35">
      ...
      <div className="relative p-5 sm:p-7 lg:p-8">
          ...
          <div className="min-w-0 space-y-5">
            ...
            <div>
              ...
              <p className="mt-4 max-w-3xl text-base leading-8 text-violet-50/74">
                {space?.description || `目标空间 ID：${spaceId || "未指定"}`}
              </p>
            </div>
        ...
        <div data-entry-surface="space-doorway" className="mt-7 grid gap-3 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)_minmax(280px,0.7fr)]">
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-4" role="tablist" aria-label="空间布局样式">
          {SPACE_LAYOUTS.map((layout: SpaceLayout) => {
...
function ActionGrid({ actions, onLayoutChange }: { actions: SpaceLayoutAction[]; onLayoutChange: (layoutId: string) => void }) {
  return (
...
function NpcRosterPanel({ space, characters, selectedCharacterId, roleplay, onSelectCharacter }: LayoutProps) {
...
function CharacterFocusCard({ space, character }: { space: Space; character?: SpaceCharacter }) {
  const avatar = characterAvatar(character)
  return (
    <Card className="relative min-w-0 overflow-hidden p-0">
      <img src={spaceStreetImage} alt="当前 NPC 背景" className="absolute inset-0 h-full w-full object-cover opacity-28" loading="lazy" decoding="async" />
      ...
      <div className="relative grid gap-5 p-5 md:grid-cols-[auto_minmax(0,1fr)_auto] md:items-center">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-violet-50/72">
            {character?.description || character?.personality || character?.first_mes || `${space.name} 还没有可展示的角色介绍。`}
          </p>
...
function MemoryFragmentsPanel({ space }: { space: Space }) {
  const previews = worldInfoPreviews(space)
  return (
...
function LobbyLayout(props: LayoutProps) {
  const { space, characters, selectedCharacterId, roleplay, stats, shareSlot, creatorSlot, onSelectCharacter, onLayoutChange } = props
    ...
    <div className="space-y-6">
      <section className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1.22fr)_minmax(320px,0.78fr)]">
        <Card className="relative min-h-[34rem] min-w-0 overflow-hidden p-0">
          <img src={spaceNightImage} alt="空间大厅" className="absolute inset-0 h-full w-full object-cover opacity-72" loading="lazy" decoding="async" />
          ...
          <div className="relative grid min-h-[34rem] content-between gap-6 p-5 sm:p-6">
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]">
              <div className="rounded-3xl border border-amber-300/18 bg-black/42 p-5 backdrop-blur-md">
                ...
                <h2 className="mt-3 text-2xl font-black text-white">{space.name} 正在营业</h2>
          ...
          <Card className="min-w-0 overflow-hidden">
            ...
            <CardContent className="space-y-3">
              ...
              <p className="rounded-2xl border border-white/10 bg-slate-950/50 p-3 text-sm leading-6 text-violet-50/70">
                {space.scene_prompt || "店主还没有写下场景提示。"}
              </p>
            </CardContent>
          </Card>
          {shareSlot}
      ...
      <section className="grid min-w-0 gap-6 xl:grid-cols-[minmax(280px,0.42fr)_minmax(0,0.58fr)]">
        ...
        <MemoryFragmentsPanel space={space} />
      </section>
    </div>
  )
}
...
function NpcChatLayout(props: LayoutProps) {
  const { space, characters, selectedCharacter, selectedCharacterId, roleplay, onSelectCharacter } = props
  ...
  return (
    <section className="grid min-w-0 gap-6 xl:grid-cols-[minmax(290px,0.42fr)_minmax(0,1fr)]">
      ...
      <div className="min-w-0 space-y-4">
        <CharacterFocusCard space={space} character={selectedCharacter} />
        <ActionGrid actions={actions} onLayoutChange={props.onLayoutChange} />
        <SpaceChat key={selectedCharacter?.id || "no-character"} space={space} character={selectedCharacter} />
      </div>
    </section>
  )
}
...
function QuestPlayLayout(props: LayoutProps) {
  const { space, selectedCharacter, stats, onLayoutChange } = props
  ...
  const worldCards = worldInfoPreviews(space)
  const fallbackCards = [
  ...
  return (
    <section className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(300px,0.42fr)]">
      <div className="min-w-0 space-y-6">
        <Card className="relative min-w-0 overflow-hidden p-0">
          <img src={spaceStreetImage} alt="委托板背景" className="absolute inset-0 h-full w-full object-cover opacity-34" loading="lazy" decoding="async" />
      ...
      <div className="min-w-0 space-y-4">
        <MemoryFragmentsPanel space={space} />
        <SpaceChat key={selectedCharacter?.id || "quest-chat"} space={space} character={selectedCharacter} />
      </div>
    </section>
  )
}
...
function HybridRoomLayout(props: LayoutProps) {
  const { space, characters, selectedCharacterId, roleplay, stats, onSelectCharacter, onLayoutChange } = props
    ...
    <div className="space-y-6">
      <section className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(320px,0.46fr)]">
        <Card className="relative min-h-[40rem] min-w-0 overflow-hidden p-0">
          <img src={spaceStreetImage} alt="混合房间" className="absolute inset-0 h-full w-full object-cover opacity-72" loading="lazy" decoding="async" />
...
export function SpaceLayoutShowcase(props: SpaceLayoutShowcaseProps) {
  const { space, spaceId, characters, roleplay, error } = props
  const initialLayoutId = normalizeSpaceLayoutStyle(space?.layout_style)
  const [activeLayoutId, setActiveLayoutId] = useState<string>(initialLayoutId)
  const activeLayout = layoutById(activeLayoutId)
  const stats = useMemo(
    () => buildSpaceLayoutStats(space, characters, roleplay?.claims || []) as SpaceLayoutStats,
    [space, characters, roleplay?.claims],
  )

  useEffect(() => {
    setActiveLayoutId(normalizeSpaceLayoutStyle(space?.layout_style))
  }, [space?.id, space?.layout_style])
