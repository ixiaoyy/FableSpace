...
import { parseCharacterCard, extractCharacterCardFromPng, getSpaceAccessLabel, getSpaceAccessIcon } from './services/spaceService'
import { addCharacter, createSpace, testLlmConfig } from '../lib/spaces'
...
import { getWizardReadiness } from './spaceCreateReadiness'
 ...
 */
export default function SpaceCreatePanel({
  initialLat = 0,
  initialLon = 0,
  ownerId = '',
  onCreated,
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const space = await createSpace(payload, ownerId)

      const addedCharacters = []
      for (const char of form.characters) {
        const addedCharacter = await addCharacter(space.id, char, ownerId)
        addedCharacters.push(addedCharacter)
      }

      if (onCreated) {
        onCreated({ ...space, characters: addedCharacters })
      }
    } catch (err) {
  ...
  function applyQuickTemplate(template, mode = 'fill') {
    ...
    const templateSpace = template?.package?.space || {}
    const templateCharacters = cloneTemplateCharacters(template)
    setForm((prev) => ({
      ...prev,
      name: overwrite || !prev.name.trim() ? (templateSpace.name || template.title || prev.name) : prev.name,
      description: overwrite || !prev.description.trim()
        ? (templateSpace.description || template.summary || prev.description)
        : prev.description,
      scene_prompt: overwrite || !prev.scene_prompt.trim()
        ? (templateSpace.scene_prompt || prev.scene_prompt)
        : prev.scene_prompt,
  ...
  const accessLabel = `${getSpaceAccessIcon(form.access)} ${getSpaceAccessLabel(form.access)}`
  ...
  return (
    <div className="space-create-panel">
      <div className="space-create-header">
        <div>
          <h3>3 分钟开一间空间</h3>
          <p className="space-create-intro">
            按顺序完成地点、空间、角色、AI 和开门检查；复杂参数之后还能在店主后台慢慢调。
          </p>
        </div>
        <div className="space-create-steps" aria-label="创建空间步骤">
      ...
      <form onSubmit={handleSubmit}>
        <div className="space-create-step-heading">
        ...
        {step === 1 && (
          <div className="space-create-step">
            <div className="location-anchor-card">
              <strong>真实地点是 FableSpace 和普通文游最大的差异。</strong>
              <p>
          ...
          <div className="space-create-step">
            <div className="quick-template-strip">
              ...
              <div className="quick-template-strip__grid">
                {SPACE_TEMPLATES.slice(0, 5).map((template) => (
              ...
              <div className="access-options">
                {['public', 'password', 'private'].map((acc) => (
                  <label key={acc} className={`access-option ${form.access === acc ? 'active' : ''}`}>
                    ...
                    />
                    <span>{getSpaceAccessIcon(acc)} {getSpaceAccessLabel(acc)}</span>
                  </label>
                ))}
              </div>
            </div>
