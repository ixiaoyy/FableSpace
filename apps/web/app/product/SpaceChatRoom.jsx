...
import SpaceContextPanel from './SpaceContextPanel'
import SpaceMemoryPanel from './SpaceMemoryPanel'
import CharacterAvatar from './CharacterAvatar'
import CharacterLookSummary from './CharacterLookSummary'
import SpaceMiniGamePanel from './SpaceMiniGamePanel'
import SpaceGameplayLauncher from './SpaceGameplayLauncher'
...
import { buildMiniGameStartPrompt, getMiniGameTemplates } from './spaceMiniGames'
import {
...
function getSpaceResponseMode(space) {
  const llmConfig = space?.llm_config && typeof space.llm_config === 'object' ? space.llm_config : {}
  ...
  if (ownerId === 'system_public_welfare' || ownerId.startsWith('system_') || spaceId.startsWith('pw_')) {
    return {
      label: '公益 LLM',
      className: 'is-rules',
      title: '公益空间',
  ...
  if (space?.status === 'closed' && !['rules', 'rule_based', 'public_welfare'].includes(backend)) {
    return {
      label: '暂不开放',
...
function hostGuideMessage(space, _characters = [], roomName = '') {
  const timestamp = Date.now()
  return {
    id: `host-guide-${timestamp}`,
    role: 'assistant',
    content: `欢迎来到${roomName || space?.name || '这间空间'}。`,
    timestamp,
...
function publicEntranceMessages(characters, space, roomName = '') {
  return [...entranceReactionMessages(characters, roomName || space?.name), hostGuideMessage(space, characters, roomName)]
}
...
function SimulationStatusBars({ state, isVisitor, currentSpaceType, lat, lon }) {
...
function CharacterSidebar({ characters, selectedChar, activeChatChannel = 'public', onSelectPublicChat, onSelectChar, space }) {
          ...
          >
            ...
            <div className="char-info">
              ...
              <SimulationStatusBars
                ...
                currentSpaceType={space?.place_type}
                lat={char.lat}
                lon={char.lon}
              />
            </div>
...
function ChatMessage({ message, character, sprites, visitorNickname, voiceConfig, spaceId, onPlayTTS }) {
        ...
        <div className="message-meta">
          ...
          {!isUser && voiceConfig?.enabled && (
            <button
              ...
              onClick={() => onPlayTTS(spaceId, message.content)}
              title="播放语音"
            >
              🔊
            </button>
    ...
    if (useBackendSTT) {
      ...
      try {
        ...
        mediaRecorder.onstop = async () => {
          ...
          try {
            const result = await transcribeVoice(spaceId || voiceConfig?.spaceId || '', blob)
...
/**
 * SpaceChatRoom — SillyTavern 风格的聊天房间
 *
 * @param {object} props
 * @param {string} props.roomId - 房间 ID（可以是 POI ID 或 space ID）
 ...
 */
export default function SpaceChatRoom({
  roomId,
  roomName = '未知地点',
  roomDescription = '',
  characters = [],
  ...
  const [gameplays, setGameplays] = useState(() => (
    Array.isArray(space?.gameplay_definitions) ? space.gameplay_definitions : []
  ))
  ...
  const groupChatEnabled = Boolean(space?.group_chat_enabled && characters.length > 1)
  const groupChatConfig = space?.group_chat_config || {}
  const spaceResponseMode = getSpaceResponseMode(space)
  ...
  const playMode = useMemo(
    () => inferSpacePlayMode(space || { characters }, selectedChar),
    [space, characters, selectedChar],
  )
  ...
  const roleplayStarterPrompts = useMemo(
    () => getRoleplayStarterPrompts({ playMode, character: selectedChar, space }),
    [playMode.id, playMode.label, playMode.summary, selectedChar?.id, space?.id, space?.scene_prompt],
  )
  const miniGameTemplates = useMemo(
    () => getMiniGameTemplates({ playModeId: playMode.id, space, character: selectedChar }),
    [playMode.id, space, selectedChar],
  )
  const guildTier = useMemo(() => getGuildTier(guildProgress), [guildProgress])
  const guildQuests = useMemo(
    () => getGuildQuestBoard(space || {}, guildProgress),
    [space, guildProgress],
  )
  ...
  useEffect(() => {
    ...
    setMessages(publicEntranceMessages(characters, space, roomName).map((message) => ({
      ...message,
      channel: 'public',
    })))
  }, [roomId, roomName, characterSignature, space?.gameplay_definitions])
  ...
  useEffect(() => {
    setGameplays(Array.isArray(space?.gameplay_definitions) ? space.gameplay_definitions : [])
  }, [space?.id, space?.gameplay_definitions])
    ...
    setMessages((prev) => {
      ...
      return [
        ...
        buildAssistantMessage({
          ...
          content: entranceReactionContent(char, roomName || space?.name),
          timestamp: Date.now(),
          character: char,
          channel: 'private',
        }),
  ...
  async function handlePlayTTS(spaceId, text) {
    if (!text || !spaceId) return
    try {
      const audioUrl = await synthesizeVoice(spaceId, { text })
      // Play the audio
    ...
    <div className="space-chat-room">
      ...
      <div className="chat-room-header">
        ...
        <div className="room-status">
          ...
          <span className={`char-badge ${spaceResponseMode.className}`} title={spaceResponseMode.title}>
            {spaceResponseMode.label}
          </span>
          <button
            type="button"
        ...
        <div className="chat-area">
          {selectedChar ? (
            <>
              ...
              <details className="space-mini-game-drawer">
