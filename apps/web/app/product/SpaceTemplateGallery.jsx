...
import { importSpacePackage } from '../lib/spaces'
import { getTemplateTags, SPACE_TEMPLATES } from './spaceTemplates'
...
export default function SpaceTemplateGallery({
  ownerId = '',
  currentLat = 0,
  currentLon = 0,
  onInstalled,
  ...
  const [selectedTemplateId, setSelectedTemplateId] = useState(SPACE_TEMPLATES[0]?.id || '')
  const selectedTemplate = SPACE_TEMPLATES.find((template) => template.id === selectedTemplateId) || SPACE_TEMPLATES[0]
  ...
  const filteredTemplates = useMemo(() => {
    ...
    return SPACE_TEMPLATES.filter((template) => {
  ...
  async function installTemplate() {
    ...
    setStatus('')
    try {
      const result = await importSpacePackage(
        {
      ...
      setStatus(`已安装模板：${space?.name || selectedTemplate.title}`)
      if (onInstalled && space) onInstalled(space)
    } catch (err) {
