 ...
 * for the current visitor in the space context.
 *
 * Props:
 ...
 */
export default function SpaceMemoryPanel({
  entryState,
  messages,
  selectedChar,
  visitorNickname,
  ...
  // Fetch memories on mount and whenever spaceId changes
  useEffect(() => {
    if (!spaceId) return
    setLoading(true)
    setError('')
    listMemories(spaceId, { visitor_id: visitorId, limit: 100 }, visitorId)
      .then((data) => {
  ...
  function handlePin(memoryId, pinned) {
    if (!spaceId) return
    togglePinMemory(spaceId, memoryId, pinned, visitorId)
      .then(() => {
        setAtoms((prev) =>
  ...
  function handleDelete(memoryId) {
    if (!spaceId) return
    deleteMemoryAtom(spaceId, memoryId, visitorId)
      .then(() => {
  ...
  function handleFeedback(memoryId, feedback) {
    if (!spaceId) return
    feedbackMemoryAtom(spaceId, memoryId, feedback, visitorId)
      .then((result) => {
