...
import { getSpaceAccessIcon, getSpaceAccessLabel, getSpaceStatusColor, getSpaceStatusLabel } from './services/spaceService'
import {
  deleteMemoryAtom,
  deleteSpace,
  exportChatHistory,
...
import SpaceCreatePanel from './SpaceCreatePanel'
...
import SpaceGroupSettingsModal from './SpaceGroupSettingsModal'
...
function buildChatTranscript(session, messages = []) {
  ...
  const header = [
    `空间：${session.space_name || session.space_id || '未知空间'}`,
...
function getSpaceTokenUsage(space) {
  return getOwnerTokenUsage(space)
}

function slugifyFilename(value) {
  const normalized = String(value || 'space')
    .trim()
    .replace(/[\\/:*?"<>|]+/g, '-')
    .replace(/\s+/g, '-')
...
/**
 * SpaceOwnerPanel — 店主管理面板
 *
 * Props:
 *   ownerId        — 店主 ID
 *   onClose        — () => void — 关闭面板
 *   onSpaceCreated — (space) => void — 空间创建/更新后回调
 *   initialTab     — number — 初始标签页（0=列表，1=创建）
 *   editSpace     — object — 初始要编辑的空间数据
 *   createTrigger  — number — 外部递增时直接打开创建向导
 */
export default function SpaceOwnerPanel({
  ownerId = '',
  onClose,
  onSpaceCreated,
  initialTab = 0,
  ...
  const [ownerSection, setOwnerSection] = useState(initialTab === 1 ? 'spaces' : 'overview')
  const [mySpaces, setMySpaces] = useState([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState(null)
  const [showCreate, setShowCreate] = useState(initialTab === 1 && !editSpace)
  const [editingSpace, setEditingSpace] = useState(editSpace)
  const [editingLlmSpace, setEditingLlmSpace] = useState(null)
  const [characterManagerSpace, setCharacterManagerSpace] = useState(null)
  const [worldBookSpace, setWorldBookSpace] = useState(null)
  const [outputRulesSpace, setOutputRulesSpace] = useState(null)
  const [promptBlocksSpace, setPromptBlocksSpace] = useState(null)
  const [presetManagerSpace, setPresetManagerSpace] = useState(null)
  const [presetImportSpace, setPresetImportSpace] = useState(null)
  const [skillPackManagerSpace, setSkillPackManagerSpace] = useState(null)
  const [groupSettingsSpace, setGroupSettingsSpace] = useState(null)
  const [stateCardSpace, setStateCardSpace] = useState(null)
  const [gameplayManagerSpace, setGameplayManagerSpace] = useState(null)
  ...
  const [chatSearchSpaceId, setChatSearchSpaceId] = useState('')
  ...
  const [spaceMetrics, setSpaceMetrics] = useState({}) // { [spaceId]: metrics }
  const [metricsLoading, setMetricsLoading] = useState(false)

  const ownerStats = useMemo(() => {
    return mySpaces.reduce(
      (stats, space) => {
        const tokenUsed = getSpaceTokenUsage(space)
        stats.total += 1
        stats.characters += space?.characters?.length || 0
        stats.worldInfo += space?.world_info?.length || 0
        stats.tokens += tokenUsed
        if (space.status === 'open') stats.open += 1
        if (space.status === 'closed') stats.closed += 1
        return stats
      },
  ...
  const tokenStats = useMemo(() => {
    return buildOwnerTokenStats(mySpaces)
  }, [mySpaces])
  ...
  const filteredSpaces = useMemo(() => {
    ...
    return mySpaces.filter((space) => {
      ...
      const matchesAccess = accessFilter === 'all' || space.access === accessFilter
      const searchable = [
        space.name,
        space.description,
        space.scene_prompt,
        space.characters?.map((character) => character.name).join(' '),
      ]
        .filter(Boolean)
        .join(' ')
        .toLowerCase()
  ...
  }, [mySpaces, statusFilter, accessFilter, searchQuery])
  ...
  const ownerSections = useMemo(() => ([
    ...
    {
      id: 'spaces',
      ...OWNER_SECTION_COPY.spaces,
      badge: `${filteredSpaces.length}/${mySpaces.length}`,
    },
    {
      id: 'visitors',
  ...
  ]), [chatStats.sessions, filteredSpaces.length, mySpaces.length, ownerStats.total, ownerStats.tokens, visitorStats.visitors])

  useEffect(() => {
    if (!showCreate && !editingSpace) {
      refreshOwnerData()
    }
  }, [showCreate, editingSpace])

  useEffect(() => {
    if (!ownerId || showCreate || editingSpace || mySpaces.length === 0) {
      setVisitorStates([])
      return
    }
    fetchOwnerVisitorStates(mySpaces)
  }, [ownerId, showCreate, editingSpace, mySpaces])

  useEffect(() => {
    if (!ownerId || showCreate || editingSpace) {
      return
    }
    fetchSpaceMetrics()
  }, [ownerId, showCreate, editingSpace, mySpaces])
  ...
  useEffect(() => {
    if (!mySpaces.length) {
      if (chatSearchSpaceId) setChatSearchSpaceId('')
      return
    }
    if (!chatSearchSpaceId || !mySpaces.some((space) => space.id === chatSearchSpaceId)) {
      setChatSearchSpaceId(mySpaces[0].id)
    }
  }, [chatSearchSpaceId, mySpaces])

  // Switch to space management when editSpace prop is provided
  useEffect(() => {
    if (editSpace) {
      setEditingSpace(editSpace)
      setShowCreate(false)
      setOwnerSection('spaces')
    }
  }, [editSpace])
  ...
  async function fetchMySpaces() {
    ...
    try {
      ...
      // Filter to only show spaces owned by this user
      setMySpaces(list.filter(t => t.owner_id === ownerId))
    } catch (err) {
  ...
  async function fetchSpaceMetrics() {
    if (!mySpaces.length) return
    setMetricsLoading(true)
    try {
      const metricsPromises = mySpaces.map(async (space) => {
        try {
          const metrics = await getSpaceMetrics(space.id)
          return { spaceId: space.id, metrics }
        } catch (err) {
          return { spaceId: space.id, metrics: null }
        }
      })
      const results = await Promise.all(metricsPromises)
      const metricsMap = {}
      results.forEach(({ spaceId, metrics }) => {
        metricsMap[spaceId] = metrics
      })
      setSpaceMetrics(metricsMap)
    } catch (err) {
      console.error('Failed to fetch space metrics:', err)
    } finally {
  ...
  async function openChatSessionDetail(session) {
    if (!session?.space_id || !session?.visitor_id) return
    ...
    try {
      const result = await getSpaceChatHistory(
        session.space_id,
        session.visitor_id,
  ...
  async function openVisitorLatestSession(visitor) {
    if (!visitor?.space_id || !visitor?.visitor_id) return
    setChatError('')
    const matchesCurrentSession = (session) => (
      session.space_id === visitor.space_id && session.visitor_id === visitor.visitor_id
    )
    ...
    try {
      const result = await listChatSessions(
        { spaceId: visitor.space_id, visitorId: visitor.visitor_id },
        ownerId,
      )
      ...
      setChatSessions((prev) => {
        const existingKeys = new Set(prev.map((session) => `${session.space_id}:${session.visitor_id}:${session.character_id}`))
        const additions = fetchedSessions.filter((session) => !existingKeys.has(`${session.space_id}:${session.visitor_id}:${session.character_id}`))
  ...
  function resolveChatSearchSession(message = {}) {
    const knownSession = chatSessions.find((session) => (
      session.space_id === message.space_id
      && session.visitor_id === message.visitor_id
      && session.character_id === message.character_id
    ))
    if (knownSession) return knownSession

    const space = mySpaces.find((item) => item.id === message.space_id)
    const character = space?.characters?.find((item) => item.id === message.character_id)
    return {
      space_id: message.space_id || '',
      space_name: space?.name || message.space_id || '未知空间',
  ...
  async function generateChatExportText() {
    ...
    try {
      const result = await exportChatHistory(
        {
          spaceId: chatDetailSession.space_id,
  ...
  async function fetchOwnerVisitorStates(spaceList = mySpaces) {
    setVisitorError('')
    if (!ownerId || !spaceList.length) {
      setVisitorStates([])
      return
    }
  ...
  async function handleToggleStatus(space) {
    const newStatus = space.status === 'open' ? 'closed' : 'open'
    try {
      await updateSpace(space.id, { status: newStatus }, ownerId)
      setMySpaces(prev => prev.map(t => t.id === space.id ? { ...t, status: newStatus } : t))
    } catch (err) {
  ...
  async function handleSaveEdit(updatedData) {
    try {
      const result = await updateSpace(editingSpace.id, updatedData, ownerId)
      setMySpaces(prev => prev.map(t => t.id === editingSpace.id ? { ...t, ...result } : t))
      setEditingSpace(null)
      if (onSpaceCreated) onSpaceCreated(result)
    } catch (err) {
  ...
  async function handleDelete(spaceId) {
    try {
      await deleteSpace(spaceId, ownerId)
      setMySpaces(prev => prev.filter(t => t.id !== spaceId))
  ...
  async function handleSaveLlm() {
    if (!editingLlmSpace || !llmFormData) return
    setSavingLlm(true)
    setLlmSaveResult(null)
    try {
      const result = await updateSpace(editingLlmSpace.id, { llm_config: llmFormData }, ownerId)
      setMySpaces(prev => prev.map(t => t.id === editingLlmSpace.id ? { ...t, ...result } : t))
      setEditingLlmSpace(prev => prev ? { ...prev, ...result } : prev)
  ...
  function handleSpaceCreated(newSpace) {
    setShowCreate(false)
    setEditingSpace(null)
    if (!mySpaces.find(t => t.id === newSpace.id)) {
      setMySpaces(prev => [newSpace, ...prev])
    } else {
      setMySpaces(prev => prev.map(t => t.id === newSpace.id ? newSpace : t))
    }
    setOwnerSection('spaces')
    if (onSpaceCreated) onSpaceCreated(newSpace)
  }

  function handleCharactersUpdated(updatedSpace) {
    setMySpaces(prev => prev.map(t => t.id === updatedSpace.id ? { ...t, ...updatedSpace } : t))
    setCharacterManagerSpace(updatedSpace)
    if (onSpaceCreated) onSpaceCreated(updatedSpace)
  }

  function handleWorldInfoUpdated(updatedSpace) {
    setMySpaces(prev => prev.map(t => t.id === updatedSpace.id ? { ...t, ...updatedSpace } : t))
    setWorldBookSpace(updatedSpace)
    if (onSpaceCreated) onSpaceCreated(updatedSpace)
  }

  function handleOutputRulesUpdated(updatedSpace) {
    setMySpaces(prev => prev.map(t => t.id === updatedSpace.id ? { ...t, ...updatedSpace } : t))
    setOutputRulesSpace(updatedSpace)
    if (onSpaceCreated) onSpaceCreated(updatedSpace)
  }

  function handlePromptBlocksUpdated(updatedSpace) {
    setMySpaces(prev => prev.map(t => t.id === updatedSpace.id ? { ...t, ...updatedSpace } : t))
    setPromptBlocksSpace(updatedSpace)
    if (onSpaceCreated) onSpaceCreated(updatedSpace)
  }

  function handlePresetApplied(updatedSpace) {
    setMySpaces(prev => prev.map(t => t.id === updatedSpace.id ? { ...t, ...updatedSpace } : t))
    setPresetManagerSpace(updatedSpace)
    if (onSpaceCreated) onSpaceCreated(updatedSpace)
  }

  function handleSkillPacksUpdated(updatedSpace) {
    setMySpaces(prev => prev.map(t => t.id === updatedSpace.id ? { ...t, ...updatedSpace } : t))
    setSkillPackManagerSpace(updatedSpace)
    if (onSpaceCreated) onSpaceCreated(updatedSpace)
  }

  function handleGroupSettingsUpdated(updatedSpace) {
    setMySpaces(prev => prev.map(t => t.id === updatedSpace.id ? { ...t, ...updatedSpace } : t))
    setGroupSettingsSpace(updatedSpace)
    if (onSpaceCreated) onSpaceCreated(updatedSpace)
  }

  function handleGameplayUpdated(updatedSpace) {
    setMySpaces(prev => prev.map(t => t.id === updatedSpace.id ? { ...t, ...updatedSpace } : t))
    setGameplayManagerSpace(updatedSpace)
    if (onSpaceCreated) onSpaceCreated(updatedSpace)
  }

  async function handleExportPackage(space) {
    if (!space?.id) return
    setPackageBusy(`export:${space.id}`)
    setPackageStatus('')
    try {
      const payload = await exportSpacePackage(space.id, ownerId)
      const filename = `${slugifyFilename(payload?.space?.name || space.name)}-fablespace-package.json`
      downloadJsonFile(filename, payload)
      setPackageStatus(`已导出《${space.name}》空间包，不包含 API Key 和访客聊天。`)
    } catch (err) {
  ...
  async function handlePackageFileChange(event) {
    ...
    try {
      ...
      const spacePayload = packageData?.space || packageData?.tavern || {}
      const defaultLat = spacePayload.lat ?? createInitialLat ?? 0
      const defaultLon = spacePayload.lon ?? createInitialLon ?? 0
      ...
      const nameInput = window.prompt('导入后的空间名称', `${spacePayload.name || '导入空间'} 副本`)
      ...
      const imported = await importSpacePackage(
        packageData,
        {
          lat: Number(latInput),
          lon: Number(lonInput),
          name: nameInput.trim() || `${spacePayload.name || '导入空间'} 副本`,
          access: 'private',
        },
        ownerId,
      )
      const newSpace = imported?.space
      if (newSpace) {
        setMySpaces(prev => [newSpace, ...prev.filter(t => t.id !== newSpace.id)])
        if (onSpaceCreated) onSpaceCreated(newSpace)
      }
      setPackageStatus(`已导入空间包：${newSpace?.name || nameInput}`)
      setOwnerSection('spaces')
    } catch (err) {
  ...
  function openLlmEdit(space) {
    setEditingLlmSpace(space)
    setLlmFormData({
      backend: space.llm_config?.backend || 'openai',
      model: space.llm_config?.model || 'gpt-4o-mini',
      api_key: '',  // Don't pre-fill for security
      base_url: space.llm_config?.base_url || '',
      temperature: space.llm_config?.temperature ?? 0.8,
      max_tokens: space.llm_config?.max_tokens ?? 4096,
      top_p: space.llm_config?.top_p ?? 1.0,
    })
    setLlmSaveResult(null)
  }

  function openLlmEditBySpaceId(spaceId) {
    const space = mySpaces.find((item) => item.id === spaceId)
    if (space) openLlmEdit(space)
  }

  function closeLlmEdit() {
    setEditingLlmSpace(null)
    setLlmFormData(null)
    setLlmSaveResult(null)
  }
    ...
    return (
      <div className="owner-create-container">
        <SpaceCreatePanel
          ...
          onCancel={() => { setShowCreate(false); setOwnerSection('spaces') }}
        />
      </div>
    )
  }
  ...
  return (
    <div className="space-owner-panel page-enter">
      ...
      {ownerSection === 'ai' && (
        <TokenUsagePanel
          ...
          onManageLlm={openLlmEditBySpaceId}
        />
      )}
      ...
      {ownerSection === 'visitors' && (
        <>
          ...
          <OwnerVisitorStatePanel
            ...
            error={visitorError}
            onRefresh={() => fetchOwnerVisitorStates(mySpaces)}
      ...
      {ownerSection === 'spaces' && (
        <OwnerSpaceManagementSection
          ...
          onManageCharacters={setCharacterManagerSpace}
          onManageWorldBook={setWorldBookSpace}
          onManageOutputRules={setOutputRulesSpace}
          onManagePromptBlocks={setPromptBlocksSpace}
          onManagePresets={setPresetManagerSpace}
          onPreviewPresetImport={setPresetImportSpace}
          onManageSkillPacks={setSkillPackManagerSpace}
          onManageGroupSettings={setGroupSettingsSpace}
          onManageGameplay={setGameplayManagerSpace}
          onManageStateCards={setStateCardSpace}
          onExportPackage={handleExportPackage}
          onDelete={(space) => setDeleteTarget(space.id)}
        />
      )}
      ...
      {characterManagerSpace && (
        <CharacterManagementModal
          space={characterManagerSpace}
          ownerId={ownerId}
          onClose={() => setCharacterManagerSpace(null)}
          onCharactersChanged={(chars) => handleCharactersUpdated({ ...characterManagerSpace, characters: chars })}
        />
      )}

      {worldBookSpace && (
        <WorldBookEditor
          space={worldBookSpace}
          ownerId={ownerId}
          onClose={() => setWorldBookSpace(null)}
      ...
      {outputRulesSpace && (
        <OutputRulesEditor
          space={outputRulesSpace}
          ownerId={ownerId}
          onClose={() => setOutputRulesSpace(null)}
      ...
      {promptBlocksSpace && (
        <PromptBlockEditor
          space={promptBlocksSpace}
          ownerId={ownerId}
          onClose={() => setPromptBlocksSpace(null)}
      ...
      {presetManagerSpace && (
        <PresetManager
          space={presetManagerSpace}
          ownerId={ownerId}
          onClose={() => setPresetManagerSpace(null)}
      ...
      {presetImportSpace && (
        <PresetImportPreviewModal
          space={presetImportSpace}
          ownerId={ownerId}
          onClose={() => setPresetImportSpace(null)}
        />
      )}

      {skillPackManagerSpace && (
        <SkillPackManager
          space={skillPackManagerSpace}
          ownerId={ownerId}
          onClose={() => setSkillPackManagerSpace(null)}
          onUpdated={handleSkillPacksUpdated}
        />
      )}

      {groupSettingsSpace && (
        <SpaceGroupSettingsModal
          space={groupSettingsSpace}
          ownerId={ownerId}
          onClose={() => setGroupSettingsSpace(null)}
          onSaved={handleGroupSettingsUpdated}
        />
      )}

      {gameplayManagerSpace && (
        <GameplayManager
          space={gameplayManagerSpace}
          ownerId={ownerId}
          onClose={() => setGameplayManagerSpace(null)}
      ...
      {stateCardSpace && (
        <OwnerStateCardPanel
          space={stateCardSpace}
          ownerId={ownerId}
          onClose={() => setStateCardSpace(null)}
        />
      )}
      ...
      {memoryModalVisitor && (
        <OwnerVisitorMemoryModal
          ...
          onPin={(id, pinned) => {
            togglePinMemory(memoryModalVisitor.space_id, id, pinned, ownerId)
              .then(() => setMemoryModalAtoms(prev => prev.map(m => m.id === id ? { ...m, pinned } : m)))
              .catch(err => alert('固定失败：' + err.message))
          }}
          onDelete={(id) => {
            deleteMemoryAtom(memoryModalVisitor.space_id, id, ownerId)
        ...
        <div className="modal-overlay" onClick={closeLlmEdit}>
          <div className="modal-content panel llm-modal" onClick={e => e.stopPropagation()}>
            <header className="modal-header">
              <h3>AI 配置 — {editingLlmSpace.name}</h3>
            ...
            <div className="llm-modal-body">
              ...
              <VoiceConfigSection
                ...
                ownerId={ownerId}
                onSave={(voiceConfig) => {
                  setMySpaces(prev => prev.map(t =>
                    t.id === editingLlmSpace.id ? { ...t, voice_config: voiceConfig } : t
                  ))
                }}
              />
            </div>
...
function OwnerSpaceManagementSection({
  loading,
  error,
  mySpaces,
  filteredSpaces,
        ...
        <div className="owner-list">
          {filteredSpaces.map(space => (
            <SpaceCard
              key={space.id}
              space={space}
              metrics={spaceMetrics[space.id]}
              onEdit={() => onEdit(space)}
              onToggleStatus={() => onToggleStatus(space)}
              onManageLlm={() => onManageLlm(space)}
              onManageCharacters={() => onManageCharacters(space)}
              onManageWorldBook={() => onManageWorldBook(space)}
              onManageOutputRules={() => onManageOutputRules(space)}
              onManagePromptBlocks={() => onManagePromptBlocks(space)}
              onManagePresets={() => onManagePresets(space)}
              onPreviewPresetImport={() => onPreviewPresetImport(space)}
              onManageSkillPacks={() => onManageSkillPacks(space)}
              onManageGroupSettings={() => onManageGroupSettings(space)}
              onManageGameplay={() => onManageGameplay(space)}
              onExportPackage={() => onExportPackage(space)}
              packageBusy={packageBusy === `export:${space.id}`}
    ...
    <section className="owner-token-panel panel" aria-label="AI 用量参考状态面板">
      ...
      <div className="owner-token-summary">
        ...
        <div>
          ...
          <strong>{tokenStats.topSpaceName || '—'}</strong>
        </div>
      </div>
      ...
      {rows.length === 0 ? (
        ...
        <div className="owner-token-list">
          {rows.map((row) => {
            const { spaceId, name, tokens, backend, model, configured, llmStatusLabel, statusLabel, usageLabel } = row
            const percent = tokenStats.topTokens ? Math.round((tokens / tokenStats.topTokens) * 100) : 0
            return (
              <article key={spaceId} className="owner-token-row">
                ...
                <div className="owner-token-row__footer">
                  ...
                  <button type="button" className="button-link" onClick={() => onManageLlm(spaceId)}>
                    AI 配置
                  </button>
                </div>
              </article>
      ...
      <div className="modal-content panel owner-chat-detail-modal" onClick={(event) => event.stopPropagation()}>
        <header className="modal-header owner-chat-detail-header">
          <div>
            ...
            <p className="note muted">
              {session.space_name || '未知空间'} · {session.message_count || messages.length || 0} 条消息 · {formatChatTimestamp(session.updated_at)}
            </p>
          </div>
      ...
      {error ? (
        ...
        <div className="owner-chat-list">
          {rows.map((session) => {
            ...
            return (
              <article
                key={`${session.space_id}-${session.visitor_id}-${session.character_id}`}
                className="owner-chat-row"
              >
                <div className="owner-chat-row__meta">
                  <div>
                    <strong>{visitorLabel}</strong>
                    <span>{session.space_name || '未知空间'} · {session.character_name || '未知角色'}</span>
                  </div>
      ...
      <form className="owner-chat-search-form" onSubmit={onSearch}>
        <label>
          ...
          <select
            ...
            disabled={!hasSpaces || loading}
          >
            {hasSpaces ? spaces.map((space) => (
        ...
        <button type="submit" className="primary" disabled={!hasSpaces || loading}>
          {loading ? '搜索中...' : '搜索聊天'}
        </button>
      </form>
      ...
      {results.length > 0 ? (
        <div className="owner-chat-search-results">
          {results.map((result) => {
            ...
            return (
              <article
                key={`${message.space_id}-${message.visitor_id}-${message.character_id}-${result.index}`}
                className="owner-chat-search-result"
              >
                <div className="owner-chat-search-result__meta">
                  <div>
                    <strong>{speaker}</strong>
                    <span>{session.space_name || '未知空间'} · {visitorLabel} · {session.character_name || '未知角色'}</span>
                  </div>
                  <time>{formatChatTimestamp(message.timestamp)}</time>
                </div>
      ...
      {error ? (
        ...
        <div className="owner-visitor-list">
          {rows.map((visitor) => {
            ...
            return (
              <article
                key={`${visitor.space_id}-${visitor.visitor_id}`}
                className="owner-visitor-row"
              >
                <div className="owner-visitor-row__main">
                  <div>
                    <strong>{visitorLabel}</strong>
                    <span>{visitor.space_name || '未知空间'} · {formatRelationshipStage(rel.stage)}</span>
                  </div>
...
function SpaceCard({
  ...
  const tokenUsed = metrics?.total_tokens || getSpaceTokenUsage(space)
  ...
  const promptBlockCount = space?.prompt_blocks?.length || 0
  const presetCount = space?.runtime_presets?.length || 0
  const skillPackCount = (space?.skill_packs || []).filter((item) => item?.enabled).length
  const gameplayCount = space?.gameplay_definitions?.length || 0
  const groupEnabled = Boolean(space?.group_chat_enabled)

  return (
    <div className={`space-card panel ${!isOpen ? 'is-closed' : ''}`}>
      <div className="space-card__header">
        <div className="space-card__info">
          <h3>
            {getSpaceAccessIcon(space.access)} {space.name}
          </h3>
          <p className="note muted">{space.description || '无描述'}</p>
        </div>
        <div className="status-badge-row">
          <span className={`status-badge ${space.status}`} style={{ borderColor: `${statusColor}55` }}>
      ...
      <div className="space-card__metrics">
        ...
        <div className="owner-metric">
          <span className="mini-label">访问权限</span>
          <strong>{getSpaceAccessIcon(space.access)} {getSpaceAccessLabel(space.access)}</strong>
        </div>
        <div className="owner-metric">
          <span className="mini-label">语音</span>
          <strong>{space.voice_config?.enabled ? '🔊 启用' : '—'}</strong>
        </div>
        <div className="owner-metric">
          <span className="mini-label">坐标</span>
          <strong>{formatCoordinate(space.lat)}, {formatCoordinate(space.lon)}</strong>
        </div>
      </div>

      {npcRankings.length > 0 && (
        <div className="space-card__npc-rankings">
...
function SpaceEditModal({ space, onSave, onClose }) {
  const [form, setForm] = useState({
    name: space.name,
    description: space.description,
    access: space.access,
    scene_prompt: space.scene_prompt,
    special_type: space.special_type || '',
    llm_config: space.llm_config || {},
  })
...
function VoiceConfigSection({ spaceId, ownerId = '', onSave }) {
  ...
  useEffect(() => {
    if (!spaceId) return
    setLoading(true)
    getVoiceConfig(spaceId, ownerId).then((data) => {
  ...
  async function handleSave() {
    ...
    setResult(null)
    try {
      await saveVoiceConfig(spaceId, config, ownerId)
  ...
  async function handleTest() {
    ...
    setResult(null)
    try {
      const audioBlob = await synthesizeVoice(spaceId, { text: '你好，这是语音测试' }, ownerId)
      ...
      <div className="modal-content panel owner-memory-modal" onClick={e => e.stopPropagation()}>
        <header className="modal-header">
          <div>
            ...
            <p className="note muted">
              {visitor.space_name || '未知空间'} · {atoms.length} 条记忆
            </p>
          </div>
          <button className="close-btn" onClick={onClose}>&times;</button>
        </header>
