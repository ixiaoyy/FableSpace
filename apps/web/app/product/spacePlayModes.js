import { FARM_PROMPTS } from './spaceFarmModes.js'
...
function collectSpaceText(space = {}, character = null) {
...
function getQuestSource(space = {}) {
  const customQuests = Array.isArray(space?.guild_quests) ? space.guild_quests : []
...
export function getGuildQuestBoard(space = {}, progress = {}) {
  ...
  return getQuestSource(space).map((quest) => ({
    ...quest,
...
export function getGuildProgressStorageKey(spaceId = 'space', visitorId = 'visitor') {
  return `fablespace_guild_progress:${spaceId || 'space'}:${visitorId || 'visitor'}`
}

export function loadGuildProgress(spaceId, visitorId) {
  if (typeof window === 'undefined' || !window.localStorage) {
    return normalizeGuildProgress()
  }
  try {
    const raw = window.localStorage.getItem(getGuildProgressStorageKey(spaceId, visitorId))
...
export function saveGuildProgress(spaceId, visitorId, progress) {
  const normalized = normalizeGuildProgress(progress)
  if (typeof window !== 'undefined' && window.localStorage) {
    try {
      window.localStorage.setItem(
        getGuildProgressStorageKey(spaceId, visitorId),
...
export function inferSpacePlayMode(space = {}, character = null) {
  const text = collectSpaceText(space, character)
  ...
  const isDivination = space.special_type === 'divination' || hasAny(text, ['占卜', '塔罗', '星象', '星盘', '命理', '测算', '神秘学', '灵签', 'tarot', 'oracle'])
...
export function getSpacePlayBadges(space = {}) {
  const mode = inferSpacePlayMode(space)
  const badges = [mode.label]
  const hasCharacters = Array.isArray(space.characters) && space.characters.length > 0
  if (hasCharacters) badges.push(`${space.characters.length} 位角色`)
  if (space.group_chat_enabled) badges.push('多人群聊')
  return badges
}
