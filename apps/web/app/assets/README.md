...
`apps/web/app/assets/` 放 Vite import 的前端运行时资产；需要被后端 seed、数据库 payload 或公开 URL 引用的资源放 `apps/web/public/assets/`。
...
- 原始草图、对比截图、contact sheet、审计图优先放 `docs/assets/<YYYY-MM-DD-topic>/` 或维护任务记录，不要散落在运行时目录。
