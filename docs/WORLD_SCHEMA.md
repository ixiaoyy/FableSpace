...
| `event_type` | `message` / `choice` / `narration` | 玩家可观察事件类型 |
...
| `payload` | object | 仅保存结构化可观察结果，不保存模型思维链 |
| `created_at` | ISO timestamp | 事件创建时间 |

自由消息只追加 `player` 与 `character` 事件，不改变节点、关键选择、故事标记或结局。`reviewed_choice` 事件必须引用当前节点中可用的发布 choice，随后才允许确定性写入关系、标记、节点与结局。
...
公开详情只返回发布 StoryWorld、Character 公开处境和固定 PlayerRole。运行时请求不接受 `player_id`；服务端从 HttpOnly、`SameSite=Lax` 的 `fablespace_player_id` Cookie 解析或生成匿名 ID，`Secure` 跟随部署会话 Cookie 配置。运行时响应不回显该 ID。

当前切片用 `player_story_states`、`story_runs`、`character_relationships` 和 `story_events` 四张表持久化。完成轮次通过最近完成记录恢复结局；重新开始创建全新 StoryRun，不复制上一轮 affinity、标记、事件或选择。
