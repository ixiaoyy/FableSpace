...
自由消息只追加消息及其来源事件，不改变节点、关键选择、故事标记、长期关系或结局。模型生成结果必须结构化区分 `dialogue`、`narration_before` 与 `narration_after`：默认情况下，Character `message` 只保存角色实际说出口的 `dialogue`；可观察动作按顺序保存为 `role=system` 的独立 `narration` 事件，不进入 Character 对话上下文，也不授予 Character 新知识。后续模型上下文中的既有 Character 消息必须重新序列化为相同的三字段 JSON；仍含动作的审核开场只提取引号内对白且不改写存量事件。模型可见历史只包含当前 Character 及其通过 `source_id` 配对的玩家事件，不得注入同一 StoryRun 中其他 Character 的对话。安妮对话只接收与玩家投影相同阶段已解锁的 CanonEntry：开场节点不得提前看到 investigation/outcome 条目，并以开场专用输出边界拦截凭空新增的具体住户、亲属、病亡先后、家庭私话或自称见闻；该窄边界不得误用于已经由审核节点引入住户见闻的后续阶段。替代 Character 事件以 `replacement_source=model_policy|input_policy` 区分来源：因模型输出不合规而产生的安全替代及其来源玩家消息不进入后续模型上下文，避免固定替代自强化；输入侧安全替代仍作为经过 JSON 序列化的有效对话历史；旧宫廷替代若缺少来源标记则因原因有歧义而保留。首次非空模型响应不满足精确 JSON 合同时可以执行一次不写状态的格式修复请求，修复结果仍须通过全部输出边界；第二次非空格式失败才使用系统审核的纯对白安全替代，修复请求失败或返回空内容仍为 `503 dialogue_unavailable`；不得把任一未校验模型文本作为 Character 消息。经历史内容审核的真人 Character 是唯一例外：其 `dialogue` 保存带固定“剧情转述（非史料原话）”前缀的第三人称转述，事件 payload 标记 `historical_projection=true`，禁止第一人称、引号和生成式动作，两个 narration 字段必须为空。`reviewed_choice` 事件必须引用当前节点中可用的发布 choice，`reviewed_decision` 必须引用相同事实下唯一命中的 DecisionRule，随后才允许在同一事务内确定性写入关系、标记、节点与结局。事件序号由持久化层分配，调用方不得自行维护第二套游标。
