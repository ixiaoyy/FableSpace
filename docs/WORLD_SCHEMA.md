...
本文档只记录当前空间平台的 durable data contract。实现字段以 `backend/src/fablemap_api/core/`、`backend/src/fablemap_api/contracts/` 和 `backend/src/fablemap_api/infrastructure/models.py` 为准；若代码与本文冲突，先修正文档或代码，不要静默漂移。

## 概念映射

| 旧概念 | 当前概念 | 说明 |
|--------|----------|------|
| Place / POI | Tavern | 地图上的可进入空间 |
| Faction | TavernCharacter | 空间内 NPC |
| World Info | WorldInfoEntry | 关键词触发的背景注入 |
| World | TavernScene / scene prompt | 空间场景与氛围 |
| Player State | VisitorState | 访客与空间 / NPC 的关系 |
| OSM Data | 地图底图 | 真实坐标锚点 |

## Tavern

Tavern 是核心实体：一个挂接真实坐标、由店主维护、可进入的空间。
...
| `place_type` | 有限枚举 | 缺省 `tavern`；`home` 默认私密 |
| `special_type` | `""` / `cultivation` / `divination` 等 | 只作为薄层类型，不改变主线边界 |
| `characters` | TavernCharacter[] | 空间 NPC |
...
TavernCharacter 是空间内 NPC，优先兼容 SillyTavern Character Card V2。

| 字段 | 类型 / 取值 | 约束 |
|------|-------------|------|
| `id` | string | 唯一 ID |
| `tavern_id` | string | 所属 Tavern |
| `name` | string | 角色名 |
...
| `current_tavern_id` / `home_tavern_id` | string? | NPC 流动相关 |
...
- 角色卡导入时，SillyTavern 字段按名称映射；FableMap 扩展字段必须保持向后兼容。
...
- 店主确认前不得进入公开 Tavern payload。
- 店主确认前不得覆盖已有 TavernCharacter。
- 店主确认前不得随空间包导出。
- 确认后按 TavernCharacter 保存；草稿本身不新增持久字段。
...
- 访客性别只作用于 `tavern_id + visitor_id`，不得扩展为全局资料或公开社交筛选。
- VisitorState 是访客私有运行时状态，不进入公开 Tavern payload。
...
GameplayDefinition 是店主发布在单个 Tavern 内的轻量玩法定义。
...
- Definition 是 Tavern 内容，可随空间包导出。
- Session 是运行时私有数据，不进入公开 Tavern payload。
...
| `canon_scope` | `visitor` / `tavern` | 作用域 |
...
- Tavern-scope 或 fixed canon 只能由店主维护。
- StateCard 不得变成 RPG 属性、装备、排行榜或公开社交资料。

## Place / Home / Relationship

Home 复用 Tavern 基础字段，仍必须有真实 `lat/lon`。
...
- `source_owner_id`
- `source_tavern_id`
- `source_node_type`: `tavern` / `character`
- `source_node_id`
- `target_owner_id`
- `target_tavern_id`
- `target_node_type`
