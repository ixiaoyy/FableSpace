...
探索在 FableMap 赛博酒馆主线中加入“抽卡 / 抽角色”式轻玩法的可行 MVP：让用户以有趣的方式发现或获得 NPC 互动入口，同时必须保留真实坐标、店主主权、SillyTavern 角色卡兼容、无平台充值/结算和无传统 RPG 化的边界。

## What I already know

* 用户明确要求使用 Trellis 头脑风暴来增加“抽卡抽角色玩法”。
* FableMap 主线是：坐标 / 定位 → 真实底图 → 浏览酒馆 → 进入酒馆 → 配置 AI NPC → 对话互动 → 写回记忆 → 回访反馈。
...
* 2026-05-06 用户补充：酒馆内可以存在隐藏角色；抽卡可随机抽到隐藏角色；抽到普通角色时增加好感度；好感度提升后逐步解锁更多互动玩法。
* 2026-05-06 用户选择混合方案：抽卡既可以抽到“酒馆卡”，也可以抽到“角色卡”；两类卡分别拥有独立好感度。
* 2026-05-06 用户选择隐藏角色解锁方式：抽中隐藏角色后，永久解锁给当前访客。
* 2026-05-06 用户选择抽卡频率：每日一次免费抽。
* 2026-05-06 用户选择权重策略：公益/系统酒馆默认采用阶段权重；店主酒馆可自由配置权重，也可选择默认阶段权重。
* 2026-05-06 用户选择酒馆卡效果：酒馆卡包含互动卡、隐藏角色线索卡、酒馆好感卡三类子类型。
...
* 不做平台绕过店主确认自动生成/发布酒馆内容、NPC、世界书或故事。
* 不做平台级 Token 充值、结算、抽成或 Token 市场。
* 不做无真实坐标的自由空间；酒馆仍必须锚定真实坐标。
* 不做战斗、等级、装备、竞技排行榜或可交易奖励。
* 访客间无边界社交仍不做；若涉及公开关系、认领、展示，必须保持酒馆内、角色内、店主审批内。
* Gameplay 会话是访客私有运行时数据，不进入公开 Tavern payload；店主内容定义可随酒馆导出。
* NPC / TavernCharacter 数据应优先兼容 SillyTavern 角色卡字段和可导出性。

## Assumptions (temporary)

* “抽卡”更适合被重新诠释为“抽取一次酒馆内的角色邂逅 / 今日值班 NPC / 互动玩法入口”，而不是付费卡池或稀有度攀比。
...
* 抽卡应能解释为赛博酒馆内的仪式（例如今日邂逅、命运牌、吧台抽签、值班轮盘），服务进入对话与回访。
...
* 抽卡结果类型分为至少两类：`tavern_card`（酒馆卡）与 `character_card`（角色卡）；两者都不代表可交易资产，只是当前酒馆内的互动入口/关系进度。
* 酒馆卡好感度表示“访客与这间酒馆的熟悉度 / 归属感”，可复用或扩展现有 `VisitorState.relationship`。
* 角色卡好感度表示“访客与某个 NPC 的熟悉度”，MVP 需要新增私有 per-character affinity 结构，或在 VisitorState 私有扩展中按 character_id 保存。
* 抽到酒馆卡时增加酒馆好感，并解锁酒馆级互动：例如隐藏角色线索、吧台仪式、回访信笺、整间酒馆的特殊入口。
* 抽到角色卡时增加该角色好感，并解锁角色级互动：例如专属问候、角色委托、私密话题、角色绑定的 Gameplay。
* 隐藏角色抽中后对当前访客永久解锁；该解锁状态是访客私有进度，不改变其他访客视图，也不把隐藏角色变成全酒馆公开角色。
* 已永久解锁的隐藏角色后续应能在该访客的酒馆角色列表 / 抽卡记录 / 专属入口中再次访问。
* 店主仍应能撤下、停用或删除隐藏角色；访客已解锁记录遇到角色被停用时展示“暂不可见/已离席”而不是泄露已删除内容。
* 抽卡频率采用“每个访客 × 每间酒馆每日一次免费抽”；每日边界按酒馆时区优先，缺失时按服务器/客户端统一日期兜底。
* 好感增量也按每日抽卡节流：每日免费抽可产生酒馆卡/角色卡好感，重复访问不能无限刷好感。
* 免费抽是产品互动节奏，不引入付费补抽、充值、购买次数或平台结算。
* 卡池权重支持两种策略：`stage_default`（阶段权重）与 `owner_custom`（店主自定义权重）。
* 公益酒馆 / 系统酒馆默认使用 `stage_default`：随着酒馆好感、角色好感或已解锁状态变化，隐藏角色/高阶互动的出现概率提高。
* 普通店主酒馆可选择 `stage_default`，也可切换为 `owner_custom` 并配置酒馆卡、普通角色卡、隐藏角色卡等条目的权重。
* 店主自定义权重仍必须受安全边界限制：不能配置付费概率、不能售卖稀有度、不能绕过店主确认自动生成角色，隐藏角色仍必须是店主已确认内容。
* 权重配置需要可解释：访客端可用“今日更容易遇见熟悉的人/有隐藏传闻”这类叙事文案，不展示商业化抽卡概率诱导。
* 酒馆卡支持三类子类型：`interaction`（酒馆级互动卡）、`hidden_clue`（隐藏角色线索卡）、`affinity_boost`（酒馆好感卡）。
* 酒馆互动卡应启动或推荐店主已发布的酒馆级 Gameplay / 回访信笺 / 吧台仪式，不创建新内容。
* 隐藏角色线索卡只给出店主确认范围内的线索或氛围提示；线索不等于直接公开隐藏角色详情，除非抽中的是隐藏角色卡本身。
* 酒馆好感卡主要提升 tavern-scoped affinity，并可推动阶段权重和酒馆级解锁。
...
* MVP 需要真实后端、API 和持久化：每日抽卡、抽卡历史、隐藏角色永久解锁、酒馆好感、角色好感、卡池配置都不能只停留在前端本地状态。
...
* `VisitorState.relationship` 保持酒馆级好感的主来源；`_gacha_progress[visitor_id].character_affinity[character_id]` 保存角色级好感。
...
* [x] 方案选择混合好感：tavern-scoped 酒馆好感 + character-scoped 角色好感并存。
* [x] 抽卡频率已确认：每个访客 × 每间酒馆每日一次免费抽；不做付费补抽。
* [x] 权重策略已确认：系统/公益酒馆默认阶段权重；店主可自定义权重或选择阶段权重。
* [ ] 方案明确酒馆卡 / 角色卡的卡池配置、好感增量和重复抽取处理。
* [x] 酒馆卡效果已确认：互动卡 / 隐藏角色线索卡 / 酒馆好感卡三类子类型。
...
1. 店主在酒馆内配置普通角色与隐藏角色；隐藏角色必须是已确认 TavernCharacter，不由平台在抽卡瞬间生成。
2. 访客在单个酒馆内抽“邂逅卡”：
...
Design tension to resolve next: existing `VisitorState.relationship` is tavern-scoped, but “抽到普通角色增加好感度”听起来更像 character-scoped。MVP 可以先用酒馆好感降低复杂度，也可以新增 per-character affinity 以提高表达力。
