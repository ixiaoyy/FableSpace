...
- 使用用户现有登录态访问安妮故事页时，不再显示“服务暂时不可用”，当前轮次
  能恢复或进入新的聊天界面。
- 获取一次新的 backend 日志，确认请求不再因 1054 缺列失败；不得用附件中的
  旧日志声称修复完成。
- 普通 push/手动部署不会自动执行 006 或 008；只有专用人工修复入口可触发。

## Acceptance Criteria

- [ ] 生产执行入口要求精确确认词，并在 DDL 前完成目标提交、迁移哈希、运行
      ORM、Schema、回填覆盖、未审核表与备份 SHA-256 检查。
- [ ] 只按顺序执行现有 006 与 008；除 008 精确清单外没有其他 Schema 变化。
- [ ] 成功后 `story_runs.player_role_id` 为 `VARCHAR(128) NOT NULL` 且数据完整。
- [ ] 成功后物理 Schema 精确为 8 张当前表，旧内联记忆列不存在。
- [ ] 当前 backend 和健康检查成功，安妮故事页在真实登录态下恢复。
- [ ] 新鲜 backend 日志不再出现本次 1054；备份路径与 SHA-256 可追踪且不泄密。
- [ ] Python/脚本静态检查、workflow 语法检查和相关构建通过。
- [ ] 用户现有 `AGENTS.md` 与 `UI稿/` 改动未修改、未暂存。
