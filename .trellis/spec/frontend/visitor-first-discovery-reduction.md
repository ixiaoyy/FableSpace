...
Use this for `/discover` first-time visitor selection flow and FableMap discover reference artboards.

## Contracts

- Default visitor `/discover` uses `visitorReduced` mode: map/search + top 3 tavern cards + one clear `进入这个空间 →` CTA per card. `?view=expanded` or owner-scoped discovery may opt into the expanded operational artboard.
...
- This reduction does not remove underlying owner-authored taverns or change API payload semantics.
