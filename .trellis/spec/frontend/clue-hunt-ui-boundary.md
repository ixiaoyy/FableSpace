...
Use this for owner clue-hunt creation in `/owner`, visitor route `/clue-hunts/:routeId`, and clue-hunt API client helpers in `frontend/app/lib/taverns.ts`.
...
- Visitor copy must state that rewards are tavern-local commemorative output, not platform wallet/recharge/ranking value.
- API calls stay centralized in `frontend/app/lib/taverns.ts`; route components must not hand-roll clue-hunt fetches.
