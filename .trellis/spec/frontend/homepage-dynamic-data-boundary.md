...
- `frontend/app/routes/home.tsx` fetches space cards, platform stats, and recent memories in parallel.
- Failure of `/api/v1/platform/stats` or `/api/v1/platform/recent-memories` must not block the space list or primary home rendering.
- API parsing stays in `frontend/app/lib/spaces.ts`; route UI must not parse `{data, meta}` envelopes directly.
- Global metrics should prefer `/api/v1/platform/stats` values and fall back to the paged space list only when the stats call fails.
