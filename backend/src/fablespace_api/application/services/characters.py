...
from fablespace_api.core.space import EXPRESSION_CATEGORIES, STANDARD_EXPRESSIONS, TavernSpriteSet
...
def _fallback_role_name(space_name: str, style_tags: list[str]) -> str:
...
def _sanitize_character_draft(data: dict[str, Any], *, style_tags: list[str]) -> dict[str, Any]:
    ...
    if not draft["system_prompt"]:
        draft["system_prompt"] = "你是 FableSpace 店主确认前的未发布 AI 草稿 NPC；店主保存后才可正式接待访客。"
    return draft
...
class CharacterApplicationMixin:
    ...
    def list_characters(self, space_id: str, user_id: str = "") -> dict[str, Any]:
        tavern = self.get_space(space_id, user_id)
        return {"characters": tavern.get("characters", [])}

    def generate_character_draft(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        ...
        space_name = clean_text(tavern.name, max_length=80) or "这间空间"
        ...
        mes_example = (
            "<START>\n"
            "{{user}}: 这里是什么地方？\n"
            f"{{{{char}}}}: 这里是{space_name}，这是店主审核前的本地模板草稿。"
            "如果你提到越界内容，我会把话题带回空间氛围和安全互动。"
        )

        draft = {
            "name": _fallback_role_name(space_name, style_tags),
            "description": (
                f"{space_name}的未发布本地模板 NPC 草稿。灵感来自空间简介：{tavern_description}。"
                f"当前风格标签：{style_text}。"
            ),
            "personality": f"说话风格偏{tone}；围绕店主输入的标签提供占位方向，等待店主改写确认。",
            "scenario": (
                f"角色暂时站在{space_name}的入口或吧台旁，等待店主审核。"
                f"空间背景：{tavern_description}"
            ),
            "system_prompt": (
                "你是 FableSpace 店主确认前的未发布本地模板草稿 NPC。保持原创，不模仿现实名人、受版权保护角色或特定 IP；"
                "不得声称自己已自动发布，也不得覆盖已有角色。店主确认保存后才可成为 TavernCharacter。"
                f"遵守禁忌方向：{forbidden_text}。保持角色扮演口吻，遇到越界请求时简短拒绝并回到空间互动。"
            ),
            "first_mes": f"欢迎来到{space_name}。我只是店主审核前的本地模板草稿，等店主确认后才会正式接待你。",
    ...
    def _generate_owner_llm_character_draft(
        ...
        *,
        ...
        system = (
            "你是 FableSpace 的 NPC 草稿助手。只返回 JSON，不要 Markdown，不要解释。"
    ...
    def add_character(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        return self.taverns.add_character(space_id, data, user_id)

    def import_character_card(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        return self.taverns.import_character_card(space_id, data, user_id)

    def update_character(self, space_id: str, char_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        return self.taverns.update_character(space_id, char_id, data, user_id)

    def delete_character(self, space_id: str, char_id: str, user_id: str = "") -> dict[str, str]:
        return self.taverns.delete_character(space_id, char_id, user_id)
    ...
    def get_character_sprites(self, space_id: str, character_id: str, user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        ...
        if space_id:
            ...
            if llm_config and llm_config.is_configured() and external_backend not in {
                ...
                try:
                    ...
                    logger.warning("Expression LLM inference failed for tavern=%s: %s", space_id, exc)
        ...
        try:
            if "json" in payload:
                ...
                if decoded.startswith(b"\x89PNG") or b"PK\x03\x04" in decoded:
                    from fablespace_api.core.char_card_parser import parse_character_card as parse_card
    ...
    def export_character_card_payload(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            from fablespace_api.core.char_card_parser import export_character_card, parse_character_card as parse_card
