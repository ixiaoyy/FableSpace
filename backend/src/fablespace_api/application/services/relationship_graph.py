...
from fablespace_api.core.relationship_graph import (
    RelationshipEdge,
...
if TYPE_CHECKING:
    from fablespace_api.infrastructure.relationship_graph_store import SQLAlchemyRelationshipGraphStore
...
class RelationshipGraphService:
    ...
    def _candidate_edges(self, event: RelationshipPropagationEvent) -> list[_CandidateEdge]:
        ...
        if event.source_node_type == "character" and event.source_space_id:
            lookup_nodes.append(("tavern", event.source_space_id))
            ...
            if self._matches_source(edge, event):
                candidates.append(
                    _CandidateEdge(
                        ...
                        affected_tavern_id=edge.target_space_id or edge.target_node_id,
                    )
                )
            if self._matches_target(edge, event):
                candidates.append(
                    _CandidateEdge(
                        ...
                        affected_tavern_id=edge.source_space_id or edge.source_node_id,
                    )
                )
        return candidates
    ...
    def _matches_source(edge: RelationshipEdge, event: RelationshipPropagationEvent) -> bool:
        ...
        parent = (
            ...
            and edge.source_node_id == event.source_space_id
        )
    ...
    def _matches_target(edge: RelationshipEdge, event: RelationshipPropagationEvent) -> bool:
        ...
        parent = (
            ...
            and edge.target_node_id == event.source_space_id
        )
        return exact or parent
    ...
    def _apply_candidate(
        ...
        current = self.store.get_projection(
            ...
            space_id=candidate.affected_tavern_id,
        )
        ...
        updated.space_id = candidate.affected_tavern_id
    ...
    def _apply_character_rollup(
        ...
        if event.source_node_type != "character" or not event.source_space_id:
            return None
        ...
        current = self.store.get_projection(event.visitor_id, "tavern", event.source_space_id) or RelationshipProjection(
            visitor_id=event.visitor_id,
            node_type="tavern",
            node_id=event.source_space_id,
            space_id=event.source_space_id,
        )
        ...
        return RelationshipPropagationResult(
            ...
            target_node_id=event.source_space_id,
            affinity=saved.affinity,
            hostility=saved.hostility,
            reason="character_rollup",
        )
...
class RelationshipGraphApplicationMixin:
    ...
    def list_relationship_edges(self, space_id: str, user_id: str) -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        owner_id = self._require_user_id(user_id)
        self._ensure_owner(tavern, owner_id)
        edges = self._relationship_graph_store().list_edges_for_source("tavern", space_id)
        return {"edges": [self._relationship_edge_payload(edge) for edge in edges], "count": len(edges)}

    def create_relationship_edge(self, space_id: str, data: dict[str, Any], user_id: str) -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        owner_id = self._require_user_id(user_id)
        self._ensure_owner(tavern, owner_id)
        payload = data or {}
        source_space_id = str(payload.get("source_space_id") or space_id).strip()
        if source_space_id != space_id:
            raise HTTPException(status_code=400, detail="只能创建当前空间 source-side 关系")

        target_space_id = str(payload.get("target_space_id") or "").strip()
        if not target_space_id:
            raise HTTPException(status_code=400, detail="target_space_id 不能为空")
        target_tavern = self._get_tavern_or_404(target_space_id)
        ...
        edge = RelationshipEdge(
            ...
            source_node_id=str(payload.get("source_node_id") or space_id),
            target_owner_id=target_tavern.owner_id,
            target_space_id=target_space_id,
            target_node_type=str(payload.get("target_node_type") or "tavern"),
            target_node_id=str(payload.get("target_node_id") or target_space_id),
    ...
    def update_relationship_edge(
        ...
        owner_id = self._require_user_id(user_id)
        self._ensure_owner(tavern, owner_id)
        edge = self._get_source_edge_or_404(space_id, edge_id)
        payload = data or {}

        if "source_space_id" in payload and str(payload.get("source_space_id") or "") != space_id:
            raise HTTPException(status_code=400, detail="不能把关系改写到其他 source tavern")
        target_space_id = str(payload.get("target_space_id") or edge.target_space_id).strip()
        target_tavern = self._get_tavern_or_404(target_space_id)
    ...
    def _get_source_edge_or_404(self, space_id: str, edge_id: str) -> RelationshipEdge:
        edge = self._relationship_graph_store().get_edge(edge_id)
        if edge is None or edge.source_space_id != space_id:
