...
from fablespace_api.core.preset_import import (
    PresetImportError,
...
class OwnerConfigApplicationMixin:
    ...
    def test_world_info(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
    ...
    def get_output_rules(self, space_id: str, user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_owner(tavern, user_id)
    ...
    def save_output_rules(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_owner(tavern, user_id)
        payload = data or {}
    ...
    def test_output_rules(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_owner(tavern, user_id)
        payload = data or {}
    ...
    def get_prompt_blocks(self, space_id: str, user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_owner(tavern, user_id)
    ...
    def save_prompt_blocks(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_owner(tavern, user_id)
        payload = data or {}
    ...
    def preview_prompt_blocks(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_owner(tavern, user_id)
        payload = data or {}
    ...
    def preview_owner_dialogue_dry_run(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        ...
        visitor_state = self.store.get_visitor_state(space_id, visitor_id) if visitor_id else None
        history = self.store.get_chat_history(space_id, visitor_id, character.id, limit=8) if visitor_id else []
        visitor_message_count = 0
        if visitor_id:
            try:
                visitor_message_count = sum(
                    int(session.get("message_count", 0) or 0)
                    for session in self.store.list_chat_sessions(space_id, visitor_id=visitor_id, limit=None)
                )
            except Exception:
    ...
    def get_runtime_presets(self, space_id: str, user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_owner(tavern, user_id)
    ...
    def save_runtime_presets(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_owner(tavern, user_id)
        payload = data or {}
    ...
    def apply_runtime_preset(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_owner(tavern, user_id)
        payload = data or {}
        ...
        space_payload = self.update_space(space_id, update_payload, user_id)
        return {
            "ok": True,
            "space_id": space_id,
            "active_preset_id": preset.get("id") or "",
    ...
    def preview_preset_import(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_owner(tavern, user_id)
        try:
    ...
    def apply_preset_import(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_owner(tavern, user_id)
        payload = data or {}
        try:
        ...
        tavern.world_info = [
            *tavern.world_info,
            *[
                WorldInfoEntry.from_dict({**entry, "space_id": space_id})
        ...
        tavern.characters = [
            *tavern.characters,
            *[
                TavernCharacter.from_dict({**character, "space_id": space_id})
    ...
    def get_owner_default_llm(self, user_id: str = "") -> dict[str, Any]:
        ...
        from fablespace_api.domain.owner_llm_policy import mask_owner_llm_config
    ...
    def save_owner_default_llm(self, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        ...
        from fablespace_api.domain.owner_llm_policy import mask_owner_llm_config, normalize_owner_llm_config
    ...
    def generate_tavern_draft(self, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        ...
        from fablespace_api.domain.owner_llm_policy import (
    ...
    def _build_tavern_draft_messages(self, data: dict[str, Any]) -> list[dict[str, str]]:
        ...
        system = (
            "你是 FableSpace 的开店草稿助手。只返回 JSON，不要 Markdown，不要解释。"
        ...
        user = (
            ...
            f"地点类型：{data.get('place_type') or 'space'}\n"
            f"风格标签：{style_tags}\n"
            f"禁止方向：{forbidden}\n"
            f"语气：{tone}\n"
