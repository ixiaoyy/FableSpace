...
class MemoryApplicationMixin:
    ...
    def list_memories(
        ...
        *,
        ...
        atoms = [
            atom
            for atom in self.store.list_memory_atoms(space_id)
    ...
    def get_memory_atom(self, space_id: str, memory_id: str, user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_visible(tavern, user_id)
        atom = self.store.get_memory_atom(space_id, memory_id)
        if not atom:
    ...
    def create_memory_atom(self, space_id: str, data: dict[str, Any], user_id: str = "") -> dict[str, Any]:
        if not user_id:
            raise HTTPException(status_code=401, detail="创建记忆需要明确用户身份")
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_visible(tavern, user_id)
        try:
            atom = memory_atom_from_payload(data or {}, space_id=space_id, user_id=user_id, now=_utc_now_iso())
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        reason = validate_memory_atom_create(atom, tavern, user_id)
        if reason:
            raise HTTPException(status_code=403, detail=reason)
        created = self.store.save_memory_atom(space_id, atom)
    ...
    def update_memory_atom(
        ...
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_visible(tavern, user_id)
        existing = self.store.get_memory_atom(space_id, memory_id)
        if not existing:
            raise HTTPException(status_code=404, detail="记忆不存在")
        ...
        if reason:
            raise HTTPException(status_code=403, detail=reason)
        saved = self.store.save_memory_atom(space_id, updated)
    ...
    def feedback_memory_atom(
        ...
        saved = self.store.save_memory_atom(space_id, existing)
        return {
            "ok": True,
            "space_id": space_id,
    ...
    def delete_memory_atom(self, space_id: str, memory_id: str, user_id: str = "") -> dict[str, Any]:
        tavern = self._get_tavern_or_404(space_id)
        self._ensure_visible(tavern, user_id)
        atom = self.store.get_memory_atom(space_id, memory_id)
        if not atom:
        ...
        deleted = self.store.delete_memory_atom(space_id, memory_id)
        if not deleted:
            raise HTTPException(status_code=404, detail="记忆不存在")
