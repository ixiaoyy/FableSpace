...
if TYPE_CHECKING:
    from .space import Tavern, TavernCharacter, NpcSimulationState
...
def decide_npc_mobility(npc: TavernCharacter, all_taverns: List[Tavern]) -> str | None:
    ...
    if not urgent_needs:
        # 如果是工作狂且不在家，有概率回家
        if "workaholic" in npc.traits and npc.current_space_id != npc.home_space_id:
            if random.random() < 0.2:
                return npc.home_space_id
        return None
    ...
    # 寻找匹配的空间
    candidates = [t for t in all_taverns if t.place_type in target_types and t.id != npc.current_space_id]
