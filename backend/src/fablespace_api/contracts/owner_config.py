...
class SpaceDraftGenerateRequest(FlexibleBody):
    """生成空间草稿请求"""
    lat: float
    lon: float
...
class SpaceDraftCharacter(BaseModel):
    """草稿中的 NPC 字段"""
    name: str
...
class SpaceDraftResponse(BaseModel):
    """空间草稿响应"""
    draft: dict[str, Any]


TavernDraftGenerateRequest = SpaceDraftGenerateRequest
TavernDraftCharacter = SpaceDraftCharacter
TavernDraftResponse = SpaceDraftResponse
