...
from fablemap_api.infrastructure.env import parse_env_file
...
DEFAULT_SOURCE_SQLITE = Path(".fablemap-api") / "fablemap.sqlite3"


def _resolve_target_url(explicit_url: str = "", *, env_file: Path = Path(".env")) -> str:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Copy FableMap runtime data from SQLite/SQLAlchemy DB to target DB")
    parser.add_argument(
        "--source-url",
        default="",
        help="Source SQLAlchemy URL. Defaults to sqlite:///.fablemap-api/fablemap.sqlite3",
    )
    parser.add_argument(
        "--source-sqlite",
    ...
    parser.add_argument("--target-url", default="", help="Target SQLAlchemy URL. Defaults to .env FABLEMAP_DATABASE_URL")
    parser.add_argument("--env-file", default=".env", help="Env file used to resolve FABLEMAP_DATABASE_URL")
