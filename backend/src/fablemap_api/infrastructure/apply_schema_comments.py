"""Apply canonical FableMap table/column comments to a MySQL database.
...
``Base.metadata`` and only comments known FableMap SQLAlchemy tables/columns.
"""
...
from fablemap_api.infrastructure.database import Base
from fablemap_api.infrastructure.env import DEFAULT_ENV_FILE, load_env_file
from fablemap_api.infrastructure.models import *  # noqa: F401,F403 - register all SQLAlchemy tables
from fablemap_api.infrastructure.schema_comments import (
    COLUMN_COMMENTS,
    TABLE_COMMENTS,
...
def _resolve_database_url(explicit_url: str | None, env_file: Path) -> str:
    ...
    url = os.environ.get("FABLEMAP_DATABASE_URL", "").strip() or os.environ.get("FABLEMAP_MYSQL_URL", "").strip()
    if not url:
        raise SystemExit("No database URL found. Set FABLEMAP_DATABASE_URL or pass --database-url.")
    return url
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Audit or apply FableMap MySQL table/column comments.")
    parser.add_argument("--database-url", default="", help="SQLAlchemy database URL. Defaults to .env FABLEMAP_DATABASE_URL.")
