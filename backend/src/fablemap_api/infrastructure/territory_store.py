"""
FableMap Territory Store — 领地存储层
...
class TerritoryStore:
    ...
    def create_territory(self, request: TerritoryClaimRequest) -> dict[str, Any]:
        ...
        if territory.tavern_id:
            tavern_map = self._read_tavern_map()
            tavern_map[territory.tavern_id] = territory_id
            self._write_tavern_map(tavern_map)
    ...
    def delete_territory(self, territory_id: str) -> bool:
        ...
        if territory.tavern_id:
            tavern_map = self._read_tavern_map()
            tavern_map.pop(territory.tavern_id, None)
            self._write_tavern_map(tavern_map)
        ...
        *,
        ...
        for t in all_territories.values():
            if owner_id and t.get("owner_id") != owner_id:
                continue
            if tavern_id and t.get("tavern_id") != tavern_id:
                continue
            if type:
    ...
    def get_territory_by_tavern(self, tavern_id: str) -> dict[str, Any] | None:
        """通过酒馆ID获取领地。"""
        tavern_map = self._read_tavern_map()
        territory_id = tavern_map.get(tavern_id)
        if not territory_id:
            return None
...
class SQLAlchemyTerritoryStore:
    ...
    def check_availability(
        ...
        from fablemap_api.core.territory import haversine_distance, TERRITORY_TYPE_META
        ...
        try:
            query = text("""
                SELECT id, owner_id, type, center_lat, center_lon, radius, status, tavern_id, name
                FROM territories
                WHERE type = :type
    ...
    def create_territory(self, request: TerritoryClaimRequest) -> dict[str, Any]:
        ...
        try:
            query = text("""
                ...
                (id, owner_id, type, center_lat, center_lon, radius, status, tavern_id, name, created_at, updated_at)
                VALUES (:id, :owner_id, :type, :center_lat, :center_lon, :radius, :status, :tavern_id, :name, :created_at, :updated_at)
            """)
    ...
    def get_territory(self, territory_id: str) -> dict[str, Any] | None:
        ...
        try:
            query = text("""
                SELECT id, owner_id, type, center_lat, center_lon, radius, status, tavern_id, name, created_at, updated_at
                FROM territories
                WHERE id = :id
            """)
    ...
    def list_territories(
        ...
        *,
        ...
        if tavern_id:
            conditions.append("tavern_id = :tavern_id")
            params["tavern_id"] = tavern_id
        if type:
            type_str = type.value if isinstance(type, TerritoryType) else type
    ...
    def get_territory_by_tavern(self, tavern_id: str) -> dict[str, Any] | None:
        """通过酒馆ID获取领地。"""
        territories, _ = self.list_territories(tavern_id=tavern_id)
    ...
    def query_nearby(
        ...
        """查询附近领地。"""
        from sqlalchemy import text
        from fablemap_api.core.territory import haversine_distance
