...
def list_world_info(request: Request, tavern_id: str = "") -> dict[str, Any]:
    return taverns_service(request).list_world_info(get_user_id(request), tavern_id=tavern_id)


@router.post("/worldinfo")
def create_world_info(request: Request, data: WorldInfoWriteRequest) -> dict[str, Any]:
    return taverns_service(request).create_world_info(data.to_payload(), get_user_id(request))


@router.put("/worldinfo/{entry_id}")
def update_world_info(request: Request, entry_id: str, data: WorldInfoWriteRequest) -> dict[str, Any]:
    return taverns_service(request).update_world_info(entry_id, data.to_payload(), get_user_id(request))
...
def delete_world_info(request: Request, entry_id: str, data: WorldInfoWriteRequest) -> dict[str, Any]:
    return taverns_service(request).delete_world_info(entry_id, data.to_payload(), get_user_id(request))


@router.post("/worldinfo/test")
def test_world_info_global(request: Request, data: WorldInfoGlobalTestRequest) -> dict[str, Any]:
    return taverns_service(request).test_world_info_global(data.to_payload(), get_user_id(request))
